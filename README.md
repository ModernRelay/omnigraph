...
- `crates/omnigraph` (package `omnigraph-db` — the one published crate, imported as `omnigraph`): storage/runtime, branching, merge, change detection, query execution, and embeddings, plus the `compiler` (schema/query parser, typechecker, catalog, IR lowering), `storage_backend` (shared local/S3 control-object storage), and `policy` (Cedar compilation and enforcement) modules
