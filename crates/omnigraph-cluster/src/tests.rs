...
use super::*;
use crate::config::{observed_streaming_digest, streaming_digest, streaming_mode_matches_desired};
...
async fn missing_derived_graph_root_marks_drifted_and_plans_creates() {
    ...
    let streaming_digest =
        validate_config_dir(dir.path()).resource_digests["streaming.knowledge"].clone();
    ...
    assert!(plan.changes.iter().any(|change| {
        change.resource == "streaming.knowledge" && change.operation == PlanOperation::Create
    }));
...
async fn seed_deletable_state(config_dir: &Path) {
    ...
    old_streaming["declaration_revision"] =
        json!(streaming_declaration_revision("old", &old_streaming_digest));
...
fn streaming_config_accepts_bool_and_rejects_non_bool() {
    ...
    assert!(
        !unmanaged
            .resource_digests
            .contains_key("streaming.knowledge")
    );
...
fn streaming_classifies_first_class_and_drain_pending_blocks() {
    ...
    assert_eq!(
        changes[0].reason.as_deref(),
        Some("streaming_drain_pending")
    );
...
async fn streaming_apply_requires_lock_confirmation_and_actor_before_graph_effects() {
    ...
    assert!(
        unconfirmed
            .diagnostics
            .iter()
            .any(|diagnostic| { diagnostic.code == "streaming_offline_confirmation_required" })
    );
    assert!(
        unconfirmed
            .diagnostics
...
async fn apply_propagates_streaming_flag_and_disable() {
    ...
    let declaration_revision = streaming_declaration_revision("knowledge", &declaration_digest);
    assert_eq!(
        state["applied_revision"]["resources"]["streaming.knowledge"]["declaration_revision"],
    ...
    stale["declaration_revision"] =
        serde_json::json!(streaming_declaration_revision("knowledge", &stale_digest));
...
"#,
    ...
    .unwrap();
    db.failpoint_stream_ingest_one_as_for_test("node:Person", &row, 0, "stream-operator")
        .await
...
"#,
    ...
    assert!(
        schema_parked.ok && !schema_parked.converged,
        "{schema_parked:?}"
    );
    ...
    assert_eq!(
        state_after_schema_attempt["applied_revision"]["resources"]["streaming.knowledge"],
        state["applied_revision"]["resources"]["streaming.knowledge"]
    );
    let live_after_schema_attempt = Omnigraph::open_read_only(graph_root.to_str().unwrap())
        .await
        .unwrap();
    assert!(
        !live_after_schema_attempt
            .schema_source()
            .contains("nickname"),
    ...
    let correction = omnigraph::db::StreamDataCorrectionRequest {
        ...
        actions: shown_page
            ...
            .iter()
            .map(
                |entry| omnigraph::db::StreamDataCorrectionAction::Withdraw {
    ...
    let replayed =
        correct_stream_data_block_config_dir(dir.path(), "knowledge", correction, options).await;
...
async fn streaming_survives_refresh_and_converges_to_engine_truth() {
    ...
    assert_eq!(
        state["applied_revision"]["resources"]["streaming.knowledge"]["declaration_revision"],
        serde_json::json!(streaming_declaration_revision("knowledge", &enabled_digest))
    );
    ...
    assert_eq!(streaming_status.status, ResourceLifecycleStatus::Drifted);
    assert!(
        streaming_status
            .conditions
...
async fn authority_retirement_preflight_requires_actor_confirmation_and_declared_graph() {
    ...
    assert!(
        missing_authority
            .diagnostics
            .iter()
            .any(|diagnostic| { diagnostic.code == "stream_authority_retirement_actor_required" })
    );
    assert!(
        missing_authority
            .diagnostics
...
fn authority_retirement_confirm_converges_applied_state_to_retired() {
    on_big_stack(|| async {
        ...
        let before_streaming = &before["applied_revision"]["resources"]["streaming.knowledge"];
        ...
        let plan =
            plan_stream_authority_retirement_config_dir(dir.path(), "knowledge", options.clone())
                .await;
        assert!(plan.ok, "{plan:?}");
        ...
        assert_eq!(
            raced_state["applied_revision"]["resources"]["streaming.knowledge"]["profile_mode"],
            json!("DISABLED")
        );
        assert_eq!(
            Omnigraph::open(graph_root.to_str().unwrap())
        ...
        assert!(
            !result.changed,
            "retry must replay the exact durable receipt"
        );
        assert_eq!(result.profile_revision, raced_profile_revision);

        let after = read_state_json(dir.path());
        let after_streaming = &after["applied_revision"]["resources"]["streaming.knowledge"];
        ...
        assert_eq!(
            refreshed_state["observations"]["graph.knowledge"]["streaming_matches_desired"],
            json!(true)
        );
...
async fn stream_block_preflight_requires_actor_confirmation_and_declared_graph() {
    ...
    assert!(!unknown_graph.ok);
    assert!(unknown_graph.diagnostics.iter().any(|diagnostic| {
        diagnostic.code == "stream_block_graph_not_declared" && diagnostic.path == "graphs.other"
    }));
    assert!(
        !dir.path().join(CLUSTER_LOCK_FILE).exists(),
        "unknown graph refusal must happen before lock acquisition"
...
async fn stream_dead_letter_preflight_requires_actor_confirmation_and_declared_graph() {
    ...
    let recorded_revision =
        stale_state["applied_revision"]["resources"]["streaming.knowledge"]["profile_revision"]
            .as_u64()
            .unwrap();
    stale_state["applied_revision"]["resources"]["streaming.knowledge"]["profile_revision"] =
        json!(recorded_revision + 1);
    fs::write(
        dir.path().join(CLUSTER_STATE_FILE),
    ...
    let stale_list =
        list_stream_dead_letters_config_dir(dir.path(), "knowledge", None, stale_options.clone())
            .await;
    ...
    let stale_export =
        export_stream_dead_letters_config_dir(dir.path(), "knowledge", None, stale_options).await;
