...
pub fn judge_concurrent(
    ...
    // Index the claim log. Committed claims MUST appear in the history;
    // Indeterminate claims (a writer died mid-op — indefinite error) MAY.
    ...
    // (a) visible-but-unclaimed + double-apply, and claim/attribution agreement.
    ...
    //     indefinite means).
    for (&(w, op), claim) in &claim_index {
        if claim.outcome == ClaimOutcome::Committed && !seen.contains(&(w, op)) {
            return Err(format!(
                "LOST UPDATE: writer {w} op {op} ({}={}) was claimed committed but \
    ...
    // (c) per-writer program order along the serialization.
...
fn writer_life(
    ...
    runtime.block_on(Box::pin(async move {
        ...
        // this life presents no further gated calls — leave the
        // draw domain BEFORE the handle drop, or a draw could name a
        // tearing-down actor and stall into an escape. (Drop is synchronous
        // and the adapter surface is async, so the drop itself cannot issue
        // gated calls.)
        if let Some(s) = &sched {
            s.finish(writer);
...
/// Set a quiesce env var only when the caller hasn't chosen one: in-suite
/// runs default to the sequential suite's single-threaded values (the
/// pinned tests share the process, and Lance/rayon pools BUILD ONCE), while
/// the sharded fleet wrapper — separate processes, nothing pinned — can
/// UNQUIESCE for genuine Lance-internal parallelism (a named blind spot:
/// quiescing serializes the very simultaneity a concurrent universe hunts).
fn env_default(key: &str, value: &str) {
    if std::env::var_os(key).is_none() {
        unsafe { crate::env_knobs::set(key, value) };
    }
}
...
pub fn run_concurrent_universe(root: &str, sc: &ConcurrentScenario) -> ConcurrentReport {
    ...
    env_default("RAYON_NUM_THREADS", "1");
    env_default("LANCE_CPU_THREADS", "1");
    env_default("LANCE_DETERMINISTIC_BACKOFF", "1");
            ...
            .spawn_scoped(scope, || {
                ...
                let report = runtime.block_on(Box::pin(async {
                    ...
                    assert_eq!(
                        unique.len(),
                        ids.len(),
                        "OCC invariant: duplicate graph_commit_id on main under \
                         concurrent writers"
                    );
