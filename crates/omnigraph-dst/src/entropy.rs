...
//! PROGRAMMATIC: [`arm`] (re)seeds the stream mid-process —
//! `run_universe` arms a per-universe stream and forces
//! `rand::rng().reseed()` so Lance's jittered retry backoff
...
//! FIRST-USE consumers need the env gate: std's `HashMap` RandomState
//! seeds once per process and cannot be re-seeded, so programmatic
//! arming governs only consumers that can be forced to re-pull
//! (ThreadRng via `reseed()`), which is exactly the backoff case.
//!
...
// TODO(#527): one process-global stream, re-armed per universe — parallel
// universes interleave draws on it. Instance-owned entropy is a v2 issue.
