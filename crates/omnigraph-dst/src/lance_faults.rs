...
//!
//! `FaultPlan` weather previously reached only omnigraph's `StorageAdapter`
//! realm (manifest / write queue / sidecars). Lance's own table IO — data
//! files, txn files, its commit protocol — resolves through the engine's
//! process-wide Lance `ObjectStoreRegistry` and had never felt an injected
//! error. This module interposes a wrapping provider over the registry's
//! `shared-memory` scheme (the registry IS the seam — zero Lance changes):
...
//! - latency charged in VIRTUAL time (`tokio::time::sleep` on the paused
//!   clock).
//!
