...
//!    vocabulary (list = the rename ledger in the 0006 README header),
...
mod tests {
    ...
    /// renamed-away campaign vocabulary (extending a rename means extending this lint
    ...
    fn banned_words_lint() {
        ...
        for dir in ["src", "tests"] {
            for entry in std::fs::read_dir(root.join(dir)).expect("read crate dir") {
        ...
        assert!(
            hits.is_empty(),
            "banned-vocabulary hits (renamed-away campaign vocabulary):\n{}",
            hits.join("\n")
        );
    }
}
