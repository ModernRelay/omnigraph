...
/// the exact leak the prepared upstream Lance deterministic-mode
/// contribution closes (RFC 0037, Determinism: what remains, item 1);
/// once it lands, assert two-run `replay_normalized` equality here.
#[test]
#[serial]
...
/// WRITE CENSUS honesty proof (an oracle must demonstrably go
/// red under seeded blindness): a write planted DIRECTLY on the store —
/// around every wrapper, recorded by nothing — must surface in the
/// bottom-count listing as a key the census never saw. This proves the
/// from-below channel actually sees bypass writes; without it the
/// zero-unrecorded green is trust, not evidence.
#[test]
#[serial]
fn dst_write_census_bottom_listing_sees_bypass_write() {
    let adapter = std::sync::Arc::new(omnigraph::storage::ObjectStorageAdapter::in_memory());
    let rt = tokio::runtime::Builder::new_current_thread()
        .enable_time()
        .build()
        .expect("runtime");
    rt.block_on(async {
        omnigraph_dst::write_census::arm();
        // The seeded blindness: a direct store write, no wrapper in the path.
        omnigraph::storage::StorageAdapter::write_text(
            adapter.as_ref(),
            "shared-memory://dst-census-blind/planted.json",
            "{}",
        )
        .await
        .expect("plant bypass write");
        let keys = adapter.dst_list_all_keys().await.expect("bottom listing");
        let recorded = omnigraph_dst::write_census::harvest().expect("census armed");
        let recorded_set: std::collections::BTreeSet<String> = recorded
            .iter()
            .map(|c| omnigraph_dst::write_census::normalize_adapter_uri(&c.uri))
            .collect();
        let orphans: Vec<&String> = keys
            .iter()
            .filter(|k| !recorded_set.contains(k.as_str()))
            .collect();
        assert_eq!(
            orphans.len(),
            1,
            "the bottom listing must expose exactly the planted bypass write; got {orphans:?}"
        );
        assert!(
            orphans[0].ends_with("planted.json"),
            "the orphan must be the planted key, got {orphans:?}"
        );
    });
}
...
fn dst_predict_triage() {
    ...
    omnigraph_dst::harness::debug_knobs::PREDICT_LOG
...
fn fleet_replay_bundle(
    ...
    omnigraph_dst::harness::debug_knobs::OP_LOG.store(true, std::sync::atomic::Ordering::Relaxed);
    let result = omnigraph_dst::harness::run_universe_caught(&replay_root, sc);
    omnigraph_dst::harness::debug_knobs::OP_LOG.store(false, std::sync::atomic::Ordering::Relaxed);
...
/// Diverged no-op-branch merge succeeds and the merged world holds
/// exactly the expected rows. The name records finding #473 (swarm
/// seed 102, minimized): this shape once failed permanently with a
/// table-version collision in the manifest publisher; this test is
...
fn dst_merge_version_collision_diverged_edge_table() {
    ...
    runtime.block_on(async move {
        ...
        // Disjoint keys, no three-way conflict: the merge must succeed.
        let outcome = Box::pin(db.branch_merge("b0", "main"))
            .await
            .expect("diverged no-op-branch merge must succeed");
        println!("diverged merge outcome: {outcome:?}");

        // The merged world: both persons present, and w7's friendship net
        // effect (added then removed on the branch) is NO edge.
        let persons = Box::pin(omnigraph_dst::fixtures::person_rows_on(&db, "main")).await;
        assert!(
            persons.iter().any(|(n, _, _)| n == "w0"),
            "person w0 missing from merged main: {persons:?}"
        );
        assert!(
            persons.iter().any(|(n, _, _)| n == "w7"),
            "person w7 missing from merged main: {persons:?}"
        );
        let knows = Box::pin(omnigraph_dst::fixtures::knows_pairs_on(&db, "main")).await;
        assert!(
            !knows.iter().any(|(f, t)| f == "w7" || t == "w7"),
            "the branch removed w7's only edge before the merge; no edge \
             touching w7 may survive it: {knows:?}"
        );
        // Dual of the absence check: the merge must not have dropped the
        // untouched fixture edges either.
        for (from, to) in omnigraph_dst::fixtures::fixture_knows() {
            assert!(
                knows.contains(&(from.clone(), to.clone())),
                "fixture edge {from}->{to} lost through the merge: {knows:?}"
            );
        }
...
/// writers race main. This is the territory the sequential suite's real
...
fn dst_seam_scheduler_bite_and_replay() {
    ...
    // TODO(#527): sched_unattributed is printed, not asserted — the
    // fail-closed assert is a v2 issue.
    println!(
...
/// SAME-RULER FLOOR CELLS (the #503 reconciliation): the
...
/// WRITE CENSUS gate — the per-PR harness-honesty check (not ignored, so
/// dst.yml's plain `cargo test` runs it on every PR): two seeded
/// universes under the count-only probe, asserting per universe the
/// census equalities (counting rows == kill counter W; every bottom-listed
/// key recorded), coverage-map currency (no UNKNOWN kinds), the multipart
/// tripwire, and — across the pair — inventory currency (no NOVEL ops).
/// Never-fired arms are deliberately NOT asserted (no single workload
/// exercises every primitive); the full classified table lives in the
/// #[ignore]d dst_write_census_report instrument.
#[test]
#[serial]
fn dst_write_census_gate() {
    ...
    for seed in [11u64, 42] {
        ...
        let r = run_universe(&format!("shared-memory://dst-census-gate-{seed}"), &sc);
        let rows = omnigraph_dst::write_census::harvest().expect("census armed");
        let mut unknown: Vec<String> = Vec::new();
        for c in &rows {
            let class = omnigraph_dst::write_census::classify(&c.uri);
            if class == "UNKNOWN" {
                unknown.push(format!("{} {}", c.op, c.uri));
            }
            observed.insert(format!("{}:{}", c.realm, c.op));
        }
        assert!(
            unknown.is_empty(),
            "seed {seed}: unknown write kinds (coverage map out of date): {unknown:?}"
        );
        assert!(
            !observed.contains("lance:put_multipart"),
            "seed {seed}: multipart upload occurred — excluded from cut coverage \
             (CUT_COVERAGE_EXCLUSIONS); build the per-part hook first"
        );
        let (adapter_keys, lance_keys) =
            omnigraph_dst::write_census::harvest_final_keys().expect("bottom listings gathered");
        omnigraph_dst::write_census::assert_reconciles(
            &format!("seed {seed}"),
            &rows,
            r.writes_observed,
            &adapter_keys,
    ...
    let (never_fired, novel) = omnigraph_dst::write_census::inventory_diff(&observed);
    assert!(
        novel.is_empty(),
        "ops observed that are missing from WRITE_PRIMITIVES (inventory stale): {novel:?}"
    );
    println!(
        "dst write census gate: 2 universes clean; {} inventory arms not fired by this workload: {:?}",
        never_fired.len(),
        never_fired
    );
    for (p, why) in omnigraph_dst::write_census::CUT_COVERAGE_EXCLUSIONS {
        println!("dst write census gate: CUT-COVERAGE EXCLUSION {p}: {why}");
    }
}

/// INSTRUMENT — WRITE CENSUS report (coverage-map classification): run
/// standard universes under the count-only kill probe (installs both
/// realms' wrappers, never dies), record every write-class call the
/// interposition sees, classify each key against the KNOWN-artifact
/// taxonomy, and print the per-universe table. UNKNOWN rows are printed
/// with their keys — each is a coverage-map work item, not an engine bug.
/// Run explicitly:
///   cargo test -p omnigraph-dst dst_write_census_report -- --ignored --nocapture
#[test]
#[serial]
#[ignore = "instrument: write census report (per-universe classification)"]
fn dst_write_census_report() {
    use std::collections::BTreeMap;
    let mut grand: BTreeMap<&'static str, usize> = BTreeMap::new();
    let mut grand_unknown: BTreeMap<String, usize> = BTreeMap::new();
    let mut grand_ops: std::collections::BTreeSet<String> = Default::default();
    for seed in [7u64, 11, 42, 77, 101] {
        omnigraph_dst::write_census::arm();
        let sc = Scenario {
            seed,
            ops: 24,
        ...
        let r = run_universe(&format!("shared-memory://dst-census-{seed}"), &sc);
        let rows = omnigraph_dst::write_census::harvest().expect("census armed");
        let counting_rows = rows.iter().filter(|c| c.counting).count();
        let adapter = rows.iter().filter(|c| c.realm == "adapter").count();
        let lance = rows.iter().filter(|c| c.realm == "lance").count();
        ...
        for c in &rows {
            ...
            *grand.entry(class).or_default() += 1;
            *by_op.entry(format!("{}:{}", c.realm, c.op)).or_default() += 1;
            if class == "UNKNOWN" {
                *unknown.entry(format!("{} {}", c.op, c.uri)).or_default() += 1;
                *grand_unknown
                    .entry(format!("{} {}", c.op, c.uri))
                    .or_default() += 1;
            }
        }
        println!(
            "dst write census: seed {seed} | wrapper rows {} (adapter {adapter} + lance {lance}) \
             | counting {counting_rows} / harness-window {} | kill-counter W {}",
            rows.len(),
            rows.len() - counting_rows,
            r.writes_observed,
        );
        for (class, n) in &by_class {
            println!("dst write census:   {class:<24} {n}");
        }
        for (op, n) in &by_op {
            println!("dst write census:   op {op:<28} {n}");
        }
        grand_ops.extend(by_op.keys().cloned());
        for (key, n) in unknown.iter().take(20) {
            println!("dst write census:   UNKNOWN row x{n}: {key}");
        }

        // CHECK 1 — the coverage map is current: no UNKNOWN artifact kinds.
        assert!(
            unknown.is_empty(),
            "seed {seed}: unknown write kinds (coverage map out of date): {:?}",
            unknown.keys().collect::<Vec<_>>()
        );
        // CHECK 2 — multipart tripwire: per-part uploads bypass the
        // interposition (initiation-only hook); the moment a workload
        // takes the multipart path, this must flip loudly so the per-part
        // refinement gets built.
        assert!(
            !by_op.keys().any(|k| k.contains("put_multipart")),
            "seed {seed}: multipart upload occurred — excluded from cut \
             coverage (CUT_COVERAGE_EXCLUSIONS); build the per-part hook \
             before trusting this workload's coverage"
        );
        // CHECK 3 — the shared reconciliation battery (counting == W,
        // non-empty Lance listing, no bypass orphans) — one
        // implementation with the per-PR gate, so the two cannot drift.
        let (adapter_keys, lance_keys) =
            omnigraph_dst::write_census::harvest_final_keys().expect("bottom listings gathered");
        println!(
            "dst write census:   bottom count: adapter {} keys, lance {} keys",
            adapter_keys.len(),
            lance_keys.len(),
        );
        omnigraph_dst::write_census::assert_reconciles(
    ...
    println!("dst write census: === grand totals across universes ===");
    for (class, n) in &grand {
        println!("dst write census:   {class:<24} {n}");
    }
    for (key, n) in grand_unknown.iter().take(40) {
        println!("dst write census:   UNKNOWN x{n}: {key}");
    }
    // Inventory diff, aggregated across the run's universes: never-fired
    // is report-only (no workload exercises every primitive); a novel op
    // is an arm the audit never saw — a staleness red.
    let (never_fired, novel) = omnigraph_dst::write_census::inventory_diff(&grand_ops);
    for p in &never_fired {
        println!("dst write census:   NEVER FIRED this run: {p}");
    }
    for (p, why) in omnigraph_dst::write_census::CUT_COVERAGE_EXCLUSIONS {
        println!("dst write census:   CUT-COVERAGE EXCLUSION {p}: {why}");
    }
    assert!(
        novel.is_empty(),
        "ops observed that are missing from WRITE_PRIMITIVES (inventory stale): {novel:?}"
...
/// SOURCE GUARD (PR #527 review P1-2): no DST runtime path may mutate the
/// process environment after startup. The pool env arrives at process
/// start (cargo config [env] / parent Command::env); per-test toggles are
/// process-static atomics (`harness::debug_knobs`).
#[test]
fn dst_source_guard_no_runtime_env_mutation() {
    let banned_set = format!("set{}", "_var");
    let banned_remove = format!("remove{}", "_var");
    for path in omnigraph_dst::detectors::crate_rs_sources() {
        let text = std::fs::read_to_string(&path).expect("read source");
        for line in text.lines() {
            // Skip pure-comment lines so docs may NAME the banned calls;
            // any other line is checked whole (a hit inside a string or
            // trailing comment fails LOUD, the right direction for a
            // guard — a silent miss is the failure mode that matters).
            if line.trim_start().starts_with("//") {
                continue;
            }
            assert!(
                !line.contains(&banned_set) && !line.contains(&banned_remove),
                "{}: runtime process-env mutation is banned in the DST crate; \
                 use process-start env (cargo config [env] / Command::env) or \
                 harness::debug_knobs instead. Found: {line:?}",
                path.display()
            );
        }
    }
