...
/// Strong HTTP `ETag` for one blob cell: an opaque digest of the identity
/// tuple `(stable_table_id, incarnation_id, table_version, row_id,
/// property)`. The stable-identity pair (not the mutable `table_key`) makes
/// the tag rename-stable and drop/re-add ABA-safe (invariant 8). Shared by
