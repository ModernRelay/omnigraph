...
//! scan). The pre-fix implementation instead performed a full cold target
//! resolve per foreign branch — manifest state scan PLUS commit-lineage scan
//! plus a schema-contract re-read/re-validate — doubling the `__manifest` read
//! slope and making deletion O(branches × history) on un-compacted graphs.
//!
...
/// fixture that is exactly 1 read iop and 1 internal open per branch; the full
/// cold resolve (state scan + lineage scan per branch) measures exactly 2 of
/// each, so a regression re-trips this gate. `SLOPE_SLACK` absorbs incidental
/// constant-ish noise without admitting a second per-branch open.
