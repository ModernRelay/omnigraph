...
/// bug 4 / iss-schema-apply-optimize-occ — lease-removal proof.
///
/// A concurrent write to an UNTOUCHED table advances the global manifest
/// version while a schema apply holds its snapshot. The old graph-global
/// "write lease" check compared the whole manifest version against the
/// pre-apply version and false-failed the apply for this. With that check gone
/// and per-table protection (held queues + per-table expected-version CAS) in
/// its place, the apply no longer cares that a disjoint table moved: the
/// publisher rebases onto the advanced manifest and the apply succeeds.
#[tokio::test(flavor = "multi_thread", worker_threads = 4)]
async fn schema_apply_succeeds_under_concurrent_write_to_untouched_table() {
    use std::sync::Arc;
    ...
    let db = Arc::new(Omnigraph::init(&uri, helpers::TEST_SCHEMA).await.unwrap());
    // Seed the TOUCHED table (Person) so the apply has a real rewrite.
    db.mutate(
        "main",
        MUTATION_QUERIES,
        "insert_person",
    ...
    // Park the apply after it has snapshotted + done its per-table commits,
    // just before the manifest publish.
    let failpoint = ScopedFailPoint::new("schema_apply.post_snapshot_pre_commit", "pause");

    let apply_db = Arc::clone(&db);
    // AddProperty on Person — a rewrite of the touched table.
    ...
    let apply = {
        ...
        tokio::spawn(async move { apply_db.apply_schema(&desired).await })
    };

    // Wait until the apply is parked: its recovery sidecar lands after the
    // snapshot and before the park, so a non-empty __recovery dir is the signal.
    let recovery_dir = dir.path().join("__recovery");
    for _ in 0..500 {
        if recovery_dir.exists()
    ...
    // Concurrent write to the UNTOUCHED Company table → advances the GLOBAL
    // manifest version while the apply is parked. This is exactly what the old
    // lease check tripped on.
    const INSERT_COMPANY: &str =
        "query insert_company($name: String) {\n    insert Company { name: $name }\n}\n";
    let company_db = Arc::clone(&db);
    company_db
        .mutate(
            "main",
            INSERT_COMPANY,
            "insert_company",
            &mixed_params(&[("$name", "acme")], &[]),
        )
        .await
        .expect("a write to an untouched table must succeed while the apply is parked");

    // Release the apply. It publishes onto the advanced manifest (Company's row
    // is disjoint; Person's expected-version CAS still matches) and succeeds.
    drop(failpoint);

    let result = apply
        .await
        .unwrap()
        .expect("schema apply must NOT fail because an untouched table's version advanced");
    assert!(result.applied, "the migration should have applied");

    // The migration persisted across reopen: re-applying the same schema is now
    // a no-op (nickname already present). And nothing recovery-shaped remains.
    let db2 = Omnigraph::open(&uri).await.unwrap();
    let reapply = db2.apply_schema(&desired).await.unwrap();
    assert!(
        !reapply.applied,
        "the nickname property should already be present after the prior apply"
    );
    assert_eq!(helpers::count_rows(&db2, "node:Company").await, 1);
    if recovery_dir.exists() {
        assert_eq!(std::fs::read_dir(&recovery_dir).unwrap().count(), 0);
    }
}

/// bug 4 / iss-schema-apply-optimize-occ — queue-ordering proof.
///
...
async fn concurrent_optimize_on_touched_table_serializes_behind_schema_apply() {
    ...
    let db = Arc::new(Omnigraph::init(&uri, helpers::TEST_SCHEMA).await.unwrap());
    // Several separate inserts → multiple Person fragments, so optimize has
    // real compaction work on the table the apply touches.
    ...
    assert!(
        ...
        "optimize of a touched table must block behind the schema apply, not race it"
    );

    // Release the apply → it publishes + drops its guards → optimize unblocks.
    drop(failpoint);

    apply
        .await
        .unwrap()
        .expect("schema apply must succeed");
    tokio::time::timeout(std::time::Duration::from_secs(30), optimize)
        .await
        .expect("optimize must complete once the apply releases the touched-table queue")
        .unwrap()
