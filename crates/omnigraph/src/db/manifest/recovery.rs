...
async fn publish_recovery_commit(
    ...
    let exact_lineage = sidecar
        ...
        .or_else(|| {
            sidecar.protocol_v13.as_deref().map(|protocol| {
...
/// the exact prior/next profile authority. Later lifecycle, drain, claim, and
/// rebind families require another explicitly versioned strand.
pub(crate) const SIDECAR_SCHEMA_VERSION: u32 = 13;
...
const STREAM_FOLD_CONFIG_VERSION: u32 = 2;
...
pub(crate) enum SidecarKind {
    ...
    StreamEnrollment,
    /// RFC-026 Phase B1 — merge one immutable MemWAL generation into the base
    /// table and advance the lifecycle HEAD witness in the same graph publish.
    StreamFold,
    /// RFC-026 F2 — append one immutable profile-management receipt and select
    /// it with the exact next graph-global stream profile.
    StreamProfileChange,
}
...
pub(crate) enum ClassificationMode {
    ...
    StreamEnrollment,
    /// Specialized exact one-generation fold classification. Generic table
    /// recovery must never interpret this roll-forward-only mode.
    StreamFold,
...
impl SidecarKind {
    ...
    pub(crate) fn is_graph_global_barrier(self) -> bool {
        matches!(
            self,
            Self::SchemaApply | Self::StreamFold | Self::StreamProfileChange
        )
    }
    ...
    pub(crate) fn classification_mode(self, schema_version: u32) -> ClassificationMode {
        match self {
            ...
            SidecarKind::StreamFold => ClassificationMode::StreamFold,
            SidecarKind::StreamProfileChange => ClassificationMode::StreamProfileChange,
        }
    }
}
...
///
/// The cut comes from one authoritative shard-manifest revision after the
/// writer has been sealed, drained, and retired. Recovery never reconstructs
/// it from the eventual MemWAL system-index snapshot and never widens it to a
/// later generation. `generation_path` is retained as evidence even though
/// base-table recovery classifies the committed transaction and merged
/// watermark rather than rescanning fresh-tier rows.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub(crate) struct RecoveryStreamFoldCut {
...
/// Schema-v11 exact, roll-forward-only MemWAL fold payload.
///
/// `prior_head` plus `prior_epoch_floor` reconstruct the exact lifecycle row
/// that must still be authoritative at arm/recovery time. The previous merged
/// generation is bound independently because the Lance base-table transaction
/// advances that physical watermark together with the row effect. Armed state
/// fixes every input and the transaction UUID. EffectsConfirmed additionally
/// binds the exact achieved HEAD, transaction, merge watermark, and manifest
/// update; recovery can also reconstruct that confirmation from an exact
/// N+1 effect after a crash in the commit→confirmation gap.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub(crate) struct RecoveryProtocolV11 {
    pub authority: RecoveryAuthorityToken,
    pub lineage: RecoveryLineageIntent,
    pub binding: super::StreamPhysicalBinding,
    pub prior_head: super::CurrentHeadWitness,
    pub prior_epoch_floor: u64,
    #[serde(default, skip_serializing_if = "Option::is_none")]
...
/// Terminal profile transition represented by a v13 receipt.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Serialize, Deserialize)]
#[serde(rename_all = "SCREAMING_SNAKE_CASE")]
pub(crate) enum RecoveryStreamProfileChangeKind {
    EnableReceipt,
    DisableReceipt,
}
...
fn validate_stream_profile_change_v13_shape(
    ...
    {
        return Err(malformed(
            "schema-v13 StreamProfileChange must target canonical main with no table pins and carry only protocol_v13"
                .to_string(),
        ));
    }
    let protocol = sidecar
...
fn classify_table_observation(
    ...
    match kind.classification_mode(schema_version) {
        ...
        ClassificationMode::StreamEnrollment
        | ClassificationMode::StreamFold
        | ClassificationMode::StreamProfileChange => IncompletePhaseB,
    }
}
...
async fn process_stream_fold_sidecar_v12(
        ...
        && observed_token.state == StreamFoldParticipantState::ExactNoEffect
    {
        ...
        matrix = classify_stream_fold_v12_effect_matrix(observed_base.state, observed_token.state);
    }
...
async fn finalize_visible_stream_profile_change_v13(
    ...
    if operation_records.iter().any(|record| {
        ...
        return Err(stream_profile_change_error(
            sidecar,
            "recovery audit row differs from the exact fixed profile outcome",
        ));
    }
    if operation_records.is_empty() {
        crate::failpoints::maybe_fail(crate::failpoints::names::RECOVERY_RECORD_AUDIT)?;
...
pub(crate) async fn ensure_read_only_schema_coherent(
    ...
    for sidecar in sidecars {
        if matches!(
            ...
            return Err(OmniError::recovery_required(
                sidecar.operation_id,
                "read-only open cannot resolve an active MemWAL enrollment/fold intent; run a read-write open to prove no effect or roll the exact stream effect forward",
            ));
        }
        if !matches!(sidecar.writer_kind, SidecarKind::SchemaApply) {
            continue;
...
"#;
    ...
    const PROFILE_TEST_DIGEST_A: &str =
        "sha256:aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa";
    const PROFILE_TEST_DIGEST_B: &str =
        "sha256:bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb";

    struct ProfileChangeFixture {
        ...
        staged_receipt: Option<crate::table_store::StagedWrite>,
        sidecar: RecoverySidecar,
        next_profile: super::super::StreamProfileEntry,
    }

    const PROFILE_CHANGE_TEST_SCHEMA: &str = r#"
node Person {
    name: String
}
"#;
