...
/// already at `to` (the roll-forward re-runs it, and since v10 both vintages
/// share one stamp, so `from == to` reads the version and writes nothing);
/// every other stamp, and a pending storage-upgrade intent, is refused.
