...
impl Omnigraph {
    ...
    async fn dispatch_fork_reclaim(
        ...
        branch: String,
        owned_tables: Vec<(String, String)>,
        schema_guard: tokio::sync::OwnedMutexGuard<()>,
        branch_guard: tokio::sync::OwnedMutexGuard<()>,
        table_guards: Vec<tokio::sync::OwnedMutexGuard<()>>,
    ) {
    ...
    /// Flip the manifest branch authority and return the deleted branch's
    /// owned `(type_key, dataset_path)` forks for the caller's post-flip
    /// reclaim.
