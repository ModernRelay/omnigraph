...
// ─── canonical blob input shape boundary ─────────────────────────────────────

/// A blob input whose struct is NOT the canonical `{data, uri}` shape is
/// refused at the reference-retaining write boundary, before any durable
/// effect. Lance would accept the 4-child logical variant and persist it
/// into the table schema, where it breaks every later keyed write (the
/// regression pinned by
/// `end_to_end::keyed_writes_survive_committed_external_blob_reference`);
/// the boundary check fails the offending write instead of poisoning the
/// table for later writers.
#[tokio::test]
async fn stage_writes_refuse_non_canonical_blob_input_shape() {
    use arrow_array::LargeBinaryArray;
    use std::sync::Arc;

    let blob_children: arrow_schema::Fields = vec![
    ...
    .into();
    let mut blob_metadata = std::collections::HashMap::new();
    blob_metadata.insert(
        "ARROW:extension:name".to_string(),
        "lance.blob.v2".to_string(),
    );
    let blob_field = Field::new("content", DataType::Struct(blob_children.clone()), true)
        .with_metadata(blob_metadata);
    let schema = Arc::new(Schema::new(vec![
        Field::new("id", DataType::Utf8, false),
        blob_field,
    ]));
    let blob_column = StructArray::new(
        blob_children,
        vec![
            Arc::new(LargeBinaryArray::from_opt_vec(vec![Some(b"payload".as_ref())])),
            Arc::new(StringArray::from(vec![None::<&str>])),
            Arc::new(UInt64Array::from(vec![Some(0_u64)])),
            Arc::new(UInt64Array::from(vec![Some(7_u64)])),
        ],
        None,
    );
    let batch = RecordBatch::try_new(
        schema,
        vec![
            Arc::new(StringArray::from(vec!["a"])) as arrow_array::ArrayRef,
            Arc::new(blob_column),
        ],
    )
    .unwrap();
    ...
    let dir = tempfile::tempdir().unwrap();
    let store = TableStore::new(dir.path().to_str().unwrap(), test_session());
    let uri = format!("{}/docs.lance", dir.path().to_str().unwrap());

    let create_err = store.stage_create(&uri, batch.clone()).await.unwrap_err();
    assert!(
        create_err.to_string().contains("canonical"),
        "stage_create must name the canonical-shape refusal: {create_err}"
    );
    assert!(
        !std::path::Path::new(&uri).exists(),
        "refusal must precede any durable effect"
    );
    ...
    let ds = TableStore::write_dataset(&uri, person_batch(&[("alice", Some(30))]))
        ...
        .unwrap();
    let pre_version = ds.version().version;
    let overwrite_err = store.stage_overwrite(&ds, batch).await.unwrap_err();
    assert!(
        overwrite_err.to_string().contains("canonical"),
        "stage_overwrite must name the canonical-shape refusal: {overwrite_err}"
    );
    let reopened = Dataset::open(&uri).await.unwrap();
    assert_eq!(
        reopened.version().version,
        pre_version,
        "refused overwrite must leave HEAD untouched"
    );
}
