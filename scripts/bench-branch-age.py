...
def main():
    ...
    parser.add_argument("--merge-preparation-width", choices=(1, 2, 4), type=int,
                        help="scoped differential control; omitted uses the production default")
    ...
    if args.merge_preparation_width is not None:
        require(all(point["scenario"] == "general-merge-updates" for point in points),
                "--merge-preparation-width requires merge-only points")
        for point in points:
            point["params"]["merge_preparation_width"] = args.merge_preparation_width
    if args.plan:
