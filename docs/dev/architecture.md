...
- `omnigraph-control-authority` — persisted cluster-lock ownership below the engine/cluster split; depends only on `omnigraph-storage`. It owns the opaque checked stopped/offline authority used by profile changes, exact `DataBlock` inspection/correction, and terminal retirement, plus served-runtime authority. Retired export is authenticated separately by the engine's selected immutable receipt and cut-membership proof.
