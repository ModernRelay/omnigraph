...
3. Run locally against RustFS/MinIO: `OMNIGRAPH_S3_TEST_BUCKET` + `AWS_*` (incl. `AWS_ENDPOINT_URL_S3`); `cargo test -p omnigraph-engine --test write_cost_s3`. Skips gracefully without the env. NOT in CI: cost gates were moved out of `rustfs_integration` (ci.yml NOTE) pending a dedicated perf harness — run on demand; the local `write_cost.rs` twin gates every PR.
