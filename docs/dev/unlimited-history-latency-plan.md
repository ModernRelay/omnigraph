...
**Tier 2 — two spawned `omnigraph` CLI processes (cross-address-space oracle, opt-out-gated).** Lives in `crates/omnigraph-cli/tests/` (where `CARGO_BIN_EXE_omnigraph` resolves). `two_process_concurrent_inserts_converge`: two `omnigraph change` invocations insert disjoint keys on `main` against one shared local-FS graph; assert both rows present and `omnigraph commit list` shows a linear chain. Honors `OMNIGRAPH_SKIP_SYSTEM_E2E=1`; asserts convergence only, never timing-sensitive version equality. This is the only tier with genuinely separate address spaces (no shared `Session`, no shared write-queue `Mutex`).
