...
| **Latency synthesis (read first for any latency work)** — both reported production problems (write + branch ops), the cost model validated against Lance source and live R2/Railway storage, what shipped vs what's left, and the bounded-vs-unlimited-history fork | [latency.md](latency.md) |
| **Unlimited-history latency plan (the live forward plan)** — goal, the gate-first phasing (U0/U1/U2 in 0.8.0), RFC-013 interleave decisions, LanceDB learnings, naming alignment, the head-pointer format change | [unlimited-history-latency-plan.md](unlimited-history-latency-plan.md) |
