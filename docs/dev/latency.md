...
- **Opening a *known* version is list-free.** `with_version(N)` / `checkout_version(N)` resolves the path via `default_resolve_version` (a couple of cheap HEADs — try the V2 filename, fall back to V1 — then read the manifest), never a `_versions/` LIST. `resolve_version_from_listing` runs only when resolving *latest*. Validated by omnigraph's `warm_read_cost.rs` (0 opens on warm repeat).
