...
- **dst.yml**: per-PR DST tier — builds `omnigraph-dst` with `RUSTFLAGS: --cfg tokio_unstable` (the crate compiles empty without it; scoped to this workflow, never the default jobs) and runs the pinned deterministic suite: every failure line carries the universe seed, so a red run is reproducible locally from the log alone.
- **dst-nightly.yml**: nightly (cron 03:00 UTC) + manual-dispatch DST volume tier — matrix-sharded deterministic and concurrent fleets over date-derived, mutually disjoint seed intervals (each night explores fresh seeds; a given date's plan is reproducible by arithmetic). Failures are logs with seed rows, not required contexts; the concurrent fleet's `wild` mode makes no replay claim.
