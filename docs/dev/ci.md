...
- **ci.yml**: classifies documentation-only changes, checks AGENTS/doc links and immutable external Action pins, runs the required three-surface graph-vocabulary guard plus Azure contract guards, compiles default features on code changes, runs the canonical `cargo test --workspace --locked --features omnigraph-engine/failpoints,omnigraph-cluster/failpoints` gate (one feature-superset build — omitting the features skips every failpoint test and builds a different fingerprint) on its configured post-merge/tag/manual events, tests the AWS server feature, checks the container and Azure bootstrap entrypoints, compiles the checked-in Azure deployment templates, and runs bucket-gated RustFS and Azurite correctness suites. There is no RFC-026/MemWAL job or abandoned v7-v19 binary build.
...
- **Windows release matrix jobs**: build `omnigraph-cli`, `omnigraph-server`, and `omnigraph-azure-admission`, verify all three executables are packaged, and smoke-test the PowerShell installer.
...
- **Azurite Azure integration**: an Azure-touching pull-request gate and a
  post-merge/tag/manual job against a digest-pinned Azurite image. It runs the
  configured storage contract, admission lease, engine recovery, cluster
  lifecycle, server, and CLI owners, rejects accidental skips, and inventories
  the container to prove control objects, Lance data, and the admission object
  used the same backend.
- **Azure static guards**: compile both Bicep templates without Azure
  credentials, exercise deployment validation and bootstrap entrypoints, build
  the bootstrap image from a non-root base, verify container/package binary
  parity, and enforce that the admission crate depends only downward on shared
  storage.
...
- **package.yml** + **omnigraph-package.yml**: manual ECR image build; emits two image tags per commit (`<sha>`, `<sha>-aws`) via CodeBuild. The checked-in reusable workflow packages the same CLI, server, and admission binaries copied by the Dockerfile.
- **publish-image.yml**: builds and pushes the public `omnigraph-server` container image to GHCR (`ghcr.io/modernrelay/omnigraph-server:<tag>`) and to Docker Hub (`docker.io/modernrelay/omnigraph-server:<tag>`, the primary anonymous-pull channel; skipped when the `DOCKERHUB_*` secrets are unset) on `v*` tag pushes, plus manual `workflow_dispatch` with an explicit existing `v*` tag to backfill an image for a past release. It compiles the CLI, server, and Azure admission wrapper inside a `rust:1-bookworm` builder container to match the Dockerfile's bookworm-slim runtime glibc, builds with `--features omnigraph-server/aws`, smoke-checks all three image binaries, and moves `latest` only on real tag pushes — never on backfill dispatches. Separate from release.yml so an image-publish failure cannot block the binary release / Homebrew chain.
