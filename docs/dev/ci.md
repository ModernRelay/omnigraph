...
- **publish-crates.yml**: on `v*` tags (or manual dispatch with an explicit tag), publishes the single [`omnigraph-db`](https://crates.io/crates/omnigraph-db) crate to crates.io. Idempotent — it checks the current registry version first, so a failed run can be re-dispatched. The other workspace crates are `publish = false` and ship only as binaries. Requires the `CARGO_REGISTRY_TOKEN` secret; exits cleanly when it is absent.
