...
| **Quiescence and rebind** | V11 profile-v2 makes graph-level `DISABLING` explicit and restart/resume-owned. V12/lifecycle-v3 + recovery-v14 implements hidden restartable empty/non-empty `OPEN -> DRAINING -> SEALED`, strictly monotonic revisions, bounded terminal receipts, current-claim authentication, and exact ordinary/drain folds. V13/recovery-v15 adds crate-private revision-fenced `SEALED -> OPEN` resume and guarded `DRAINING -> OPEN` abort with a higher-epoch claim and terminal receipts. V14/recovery-v16 adds the checked same-binding EnsureIndices capability while all enrolled productive tables are exactly `SEALED`; current v15/recovery-v17 adds the distinct checked Optimize capability. Both re-prove lifecycle authority in the same CAS and have no receipt/operation-ID layer. The production profile adapter still completes disable only with no lanes or already-`SEALED` lanes because no supported control invokes those hidden paths. Physical rebind remains separate inactive F3 work. Resume rechecks the narrow main-only/no-named-branch topology; an incompatible branch operation leaves the stream `SEALED`. Ambient maintenance remains refused on enrolled tables. |
...
2. keep Phase A's historical v7 enrollment recovery, all-lifecycle effect
   ...
   and refusal/rebuild gates as the foundation preserved by current v15;
