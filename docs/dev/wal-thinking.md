...
| **Quiescence and rebind** | V11 profile-v2 makes graph-level `DISABLING` explicit and restart/resume-owned. V12/lifecycle-v3 + recovery-v14 implements hidden restartable empty/non-empty `OPEN -> DRAINING -> SEALED`, strictly monotonic revisions, bounded terminal receipts, current-claim authentication, and exact ordinary/drain folds. Current v13/recovery-v15 adds crate-private revision-fenced `SEALED -> OPEN` resume and guarded `DRAINING -> OPEN` abort with a higher-epoch claim and terminal receipts. The production profile adapter still completes disable only with no lanes or already-`SEALED` lanes because no supported control invokes those hidden paths. The same-binding maintenance bridge and physical rebind remain separate inactive F3 work. Resume rechecks the narrow main-only/no-named-branch topology; an incompatible branch operation leaves the stream `SEALED`. `SEALED` currently permits native branch control, not general in-place maintenance. |
...
2. keep Phase A's historical v7 enrollment recovery, all-lifecycle effect
   ...
   and refusal/rebuild gates as the foundation preserved by current v13;
