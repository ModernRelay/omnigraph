...
**Status:** Partially implemented — the server-side surface (Streamable-HTTP transport, built-in tools, stored-query tools, resources, Cedar-filtered list/call) shipped in [omnigraph#157](https://github.com/ModernRelay/omnigraph/pull/157). Deferred: per-query `invoke_query` scope (PR 0b), the OAuth/RFC-9728 layer (MR-956), and the stdio→proxy collapse.
**Date:** 2026-06-01
