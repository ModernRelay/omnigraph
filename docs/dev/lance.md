...
- **`WriteParams::auto_cleanup` default flipped from on (every-20-commits) to `None`** (PR #6755). On 6.0.1 the on-by-default hook could GC versions the `__manifest` pins for snapshots/time-travel. omnigraph owns cleanup explicitly (`optimize.rs::cleanup_all_tables`) and now pins `auto_cleanup: None` at all 11 production `WriteParams` sites — no dependency on the upstream default, and the snapshot-safe behavior locked in regardless of future re-flips.
