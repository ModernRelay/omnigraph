...
Touching `crates/omnigraph/src/storage_backend.rs`, the engine compatibility facade
...
- **object_store 0.13 moved convenience methods behind a new `ObjectStoreExt` trait** (`get`/`put`/`head`/`rename`/`delete`; `list`/`list_with_delimiter`/`put_opts` stay on the core `ObjectStore` trait). Fix = add `use object_store::ObjectStoreExt;` to the implementation now at `crates/omnigraph/src/storage_backend.rs` and to `db/manifest/namespace.rs`; no call-site changes. Mirrors Lance's own migration in PR #6672. The local-FS `PutMode::Update` gap is unchanged (still unimplemented upstream), so `omnigraph::storage_backend::ObjectStorageAdapter::write_text_if_match`'s local content-token emulation stays; `crates/omnigraph/src/storage.rs` is the engine compatibility facade.
