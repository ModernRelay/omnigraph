...
(`omnigraph:internal_schema_version`, currently v15) and **strict
...
recovery-v16 checked `SEALED` EnsureIndices bridge. Current v15 adds the
distinct recovery-v17 checked `SEALED` Optimize bridge; both refresh pointers
and lifecycle proof atomically without a token receipt or operation-ID layer,
while physical rebind remains inactive.
...
refused under current v15. Recovery-v13 `StreamProfileChange` owns one exact
...
introduces no token receipt or caller operation ID, and physical rebind remains
inactive.
...
EnsureIndices for exactly `SEALED` productive tables; current
v15/recovery-v17 adds the distinct checked Optimize capability. Both refresh
lifecycle proof atomically with natural no-work idempotency and no receipt or
operation-ID framework. Historical v10 enrollment, v12
...
v14/recovery-v16 checked `SEALED` EnsureIndices, and
v15/recovery-v17 checked `SEALED` Optimize implemented; managed reclamation is
optional later work; the remaining F3 work keeps v14's older incomplete
scaffolds frozen and finalizes distinct rebind and
...
4. **Which capability owns the next rebuild boundary after v12?** RFC-028
   ...
   achieved table HEADs bound into recovery. Physical rebind remains later F3
   work.
