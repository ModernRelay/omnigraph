...
(`omnigraph:internal_schema_version`, currently v16) and **strict
...
Current v16 adds the separate recovery-v18 private physical-rebind owner for
an exact `SEALED` lane; public/production rebind remains inactive.
...
refused under current v16. Recovery-v13 `StreamProfileChange` owns one exact
...
Current v16/recovery-v18 separately adds checked offline physical rebind from
one exact `SEALED` binding to a fresh empty scope that remains `SEALED`.
Historical v10 enrollment, v12 lifecycle-v2 folds, and the older v14
resume/maintenance/rebind scaffolds are refused rather than reinterpreted.
Older formats cross through the same export/init/load rebuild rule.
...
v15/recovery-v17 checked `SEALED` Optimize, and v16/recovery-v18 checked
offline physical rebind implemented; managed reclamation is optional later
work; the remaining F3 work keeps v14's older incomplete scaffolds frozen and
finalizes correction and terminal-`WITHDRAWN` retirement recovery before
public enrollment, quiesce,
...
4. **Which capability owns the next rebuild boundary after v12?** RFC-028
   ...
   `SEALED` binding to a fresh scope that remains `SEALED`; correction and
   terminal `WITHDRAWN` retirement remain later F3 work.
