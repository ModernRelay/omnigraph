...
(`omnigraph:internal_schema_version`, currently v14) and **strict
...
recovery-v15 private resume and guarded drain-abort. Current v14 adds the
narrow recovery-v16 checked `SEALED` EnsureIndices bridge; it refreshes pointer
and lifecycle proof atomically without a token receipt or operation-ID layer,
while Optimize and rebind remain inactive.
...
it. Explicit production enrollment/quiesce, `SEALED -> OPEN`, bounded
...
refused under current v14. Recovery-v13 `StreamProfileChange` owns one exact
...
manifest schema (`MIN_SUPPORTED == CURRENT == 14`). An older graph is refused
...
resume and guarded drain-abort. Current v14/recovery-v16 adds only checked
canonical-main EnsureIndices for exactly `SEALED` productive tables, with
atomic lifecycle-proof refresh and natural no-work idempotency; it adds no
receipt or operation-ID framework. Historical v10 enrollment, v12
lifecycle-v2 folds, and the v14 resume/maintenance scaffolds are refused rather than
reinterpreted. Older formats cross through the same export/init/load rebuild
rule.
...
durable manifest authority read-only. Explicit production enrollment/quiesce,
correction/retirement, exclusive-cut physical status, public row admission,
and CLI/HTTP/OpenAPI parity remain inactive. The
...
v13/recovery-v15 private resume/guarded drain-abort, and narrow
v14/recovery-v16 checked `SEALED` EnsureIndices implemented; managed
reclamation is optional later work; the remaining F3 work keeps v14's older
incomplete scaffolds frozen and finalizes distinct Optimize/rebind and
correction/retirement recovery shapes before public enrollment, quiesce,
...
4. **Which capability owns the next rebuild boundary after v12?** RFC-028
   ...
   Optimize and physical rebind remain later F3 work.
