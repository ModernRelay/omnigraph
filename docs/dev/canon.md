...
manifest-only. F6b7 separately adds a failpoints-only paired selected token-
...
repair, operational-status transport, and SDK/HTTP/OpenAPI
lifecycle/maintenance parity remain inactive. F6b6's internal checked status core observes one read-only authority
...
4. **Which capability owns the next rebuild boundary after v12?** RFC-028
   ...
   public status/driver transport and the F6
   guardrail remainder stay later.
