...
- **Private B2 token/fold core and B2a retain-all profile are implemented;
  ...
  fail-closed until a producer and evidence grammar exist. F6 still owns the
  dead-letter object/RSS and selected-token lookup measurements plus the full
  guardrail matrix; the v19 terminal protocol itself is active behind the
  hidden row path.
