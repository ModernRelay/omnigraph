...
| Schema apply concurrency | Acquires the touched-existing-table write queues (`rewritten ∪ rename-source ∪ dropped`) **before** the snapshot — same queue-before-read order as `optimize` — and publishes with a per-table expected-version CAS. The old graph-global "write lease" check is gone: a concurrent compaction of an *untouched* table no longer false-fails the apply (it's a disjoint manifest row the publisher rebases onto); a compaction of a *touched* table serializes behind the apply. In-process serialization is by construction; the per-table CAS is the loud cross-process backstop (`bug 4 / iss-schema-apply-optimize-occ`) | [writes.md](writes.md) |
