...
8. **Schema identity survives renames.** Accepted type and property identities
   ...
   selected frozen branch member beside that root receipt. Current v18 raises
...
| Streaming ingest | RFC-026 adopts Lance MemWAL as the strategic substrate and remains Draft; public row streaming is inactive. Historical schema v7 introduced recoverable main-only empty enrollment, v8/config-v2/recovery-v11 added the bounded B1 worker, and v9/config-v3/state-v2/recovery-v12 activated the private compare-and-chain/token-fold core. V10 added the graph-global profile singleton; v11 replaced it with checked protocol-v2 profile authority and exact recovery-v13 `StreamProfileChange`. V12 replaces inline lifecycle history with lifecycle-v3 fixed-size binding/management/claim ledger heads and activates recovery-v14 `StreamEnrollmentV2`, `StreamClaim`, `StreamFoldV2`, `StreamDrainFold`, and `StreamLifecycleReceipt`. V13 activates recovery-v15 `StreamResume`: a revision-fenced `SEALED → OPEN` resume or guarded `DRAINING → OPEN` abort owns its higher-epoch physical claim and terminal claim/management receipts before the sole `OPEN` publication. V14 activates recovery-v16 `StreamSealedEnsureIndices`; v15 activates the separate recovery-v17 `StreamSealedOptimize`. Under `stream_manage` and the exact checked runtime, canonical-main productive `SEALED` tables publish maintenance pointers and refreshed lifecycle proofs in one CAS, without a token row, management receipt, or caller operation ID; true no-work retries are effect-free. V16 activates recovery-v18 `StreamRebind`: one exact `SEALED` lane moves to a fresh enrollment and empty shard namespace with immutable binding and fence-only claim receipts, then publishes a new exact proof while remaining `SEALED`. V17 activates recovery-v19 `StreamAuthorityRetirement`: under state-lock-held stopped/offline cluster authority, an exact `DISABLED`, all-`SEALED`, recovery-clear plan with base/token parity and current `WITHDRAWN` authority appends one immutable receipt and publishes lineage-neutral `RETIRED`. The source then permits only query/status/export, and retired export carries the verified receipt as provenance. V18 activates recovery-v20 `StreamCorrection`: one exact blocked generation and selected claim bind the reconstructed winner set, pre-minted base effect, combined token-plus-receipt effect, `DRAINING` successor, system fold lineage, and operator-authenticated correction/management receipts. Ambient EnsureIndices and Optimize remain refused for enrolled tables. Older resume/maintenance/correction/retirement/rebind scaffolds remain frozen and fail closed. Every cold opener and fold first publishes an exact claim whose receipt chain authenticates the bounded WAL suffix and full current-generation LWW projection. Exclusive admission then supports restartable hidden `OPEN → DRAINING → SEALED` quiescence for empty and non-empty lanes; restart after a terminal claim reclaims with a higher epoch, while restart after physical seal reuses the exact flushed cut. Base and token effects publish only as one exact joint outcome with lifecycle, lineage, and attribution. Historical v10 enrollment, v12 lifecycle-v2 fold, and v14 resume payloads retain their old wire meaning and are refused. Admission recaptures durable authority after waiting; claims, folds, resume, rebind, and checked maintenance additionally hold profile-shared then schema → branch → stream-token → table gates across final recapture, effect, and publication. The manifest-selected `_stream_tokens.lance` witness—not its raw HEAD—is authority. Enabled-profile ordinary content writes require exact checked cluster runtime authority; BranchMerge remains closed under `ENABLED` and `DISABLING`. Hidden F4 now owns caller-shaped authorized JSON/NDJSON plus lazy-enrollment prepare. Format-neutral F5a adds a process-local, root-scoped automatic supervisor for `OPEN` lanes: checked-runtime/profile-bound timer and cap wakes coalesce by table identity, cold start derives backlog from manifest plus authenticated MemWAL authority, finite rounds visit nodes before edges, and bounded shutdown leaves invoked work recovery-v14-owned. F5b0 extends that same exact-`ENABLED` resident owner to unblocked `DRAINING(goal = SEALED)` continuation. Checked offline disable now publishes `DISABLING` first, derives a finite manifest lane cut, drains nodes before edges in deterministic identity order one lane at a time, adopts `OPEN_AFTER_FOLD` through the existing recovery-v14 lifecycle-receipt owner, and reaches terminal `DISABLED`; a selected `DataBlock` instead leaves a loud, durable pending revision for stopped/offline correction and apply retry. The two owners do not overlap, and F5b0 adds no format or public API. There is still no public ingress, production enrollment/quiesce/resume/abort/rebind, public driver health/backlog status, generation GC, fresh read, or streaming SDK/HTTP/OpenAPI surface. In addition to checked profile control through `cluster apply`, the narrow cluster/offline emergency exits are retirement and `cluster stream block show|correct`; no active path produces reserved `AuthorityBlock`, so its repair remains fail-closed, and F5b `DEAD_LETTERED` authority remains inactive. | [RFC-026](../rfcs/0026-memwal-streaming-ingest.md), [writes.md](writes.md) |
...
- **Private B2 token/fold core and B2a retain-all profile are implemented;
  ...
  selected-branch-member witness. Current v18 adds recovery-v20 exact-cut
  DataBlock correction through the state-lock-held, stopped/offline cluster
  surface. Public/production rebind remains inactive.
  Production enrollment, quiesce/resume/abort, general lifecycle control,
  ...
  and exact `DataBlock` show/correct commands are active emergency controls.
  ...
  rerun. This adds neither a public API nor persisted grammar. Public driver
  status remains future. No active path produces `AuthorityBlock`, so its repair remains
  fail-closed until a producer and evidence grammar exist; F5b's
  `DEAD_LETTERED` disposition remains future.
...
- **Branch-merge manifest history amplification:** coordinator open/full-refresh
  ...
  therefore research-blocked and the current development format remains on the v18 journal
