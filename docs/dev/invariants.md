...
8. **Schema identity survives renames.** Accepted type and property identities
   ...
   recovery-v12 B2 compare-and-chain/fold core; and the currently served v10
   format adds the required graph-global `stream_profile` enablement singleton
   (§4.7 P1 — present from genesis, disabled, single-CAS flips with strict
   revision advance) and reserves the fold-attribution dead-letter slot. See
...
- **Branch-merge manifest history amplification:** coordinator open/full-refresh
  ...
  therefore research-blocked and production remains on the current v10 journal
