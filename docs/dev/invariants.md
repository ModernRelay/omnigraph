...
8. **Schema identity survives renames.** Accepted type and property identities
   ...
   reserved the fold-attribution dead-letter slot; and the currently served
   v11 format replaces that boolean with protocol-v2
   ...
   for the sole active `StreamProfileChange` envelope. See
...
| Streaming ingest | RFC-026 adopts Lance MemWAL as the strategic substrate and remains Draft; public row streaming is inactive. Historical schema v7 implemented recoverable main-only empty enrollment, and historical v8/config-v2/recovery-v11 added the private B1 worker and one-generation fold mechanics. Schema v9/config-v3/state-v2 activated the private B2 token/fold slice behind the feature-gated, doc-hidden seam; v10 added the required graph-global `stream_profile` singleton. Canonical payload and compare-and-chain token digests bind table identity, logical key, accepted schema, stream incarnation, predecessor, write ID, trusted contributor, and payload; hidden trusted metadata follows the row; and same-generation overlays preserve idempotency and chaining before fold. Admission treats any pre-wait capture as provisional, then recaptures schema, lifecycle, binding, HEAD, and token authority after shared admission and same-key queue ownership, before `put_no_wait`. The graph-global `_stream_tokens.lance` dataset is durable but its raw HEAD is never authority; `__manifest` selects its exact witness. Recovery-v12 remains byte-for-byte the ordinary private B2 fold: it owns exact pre-minted base and token transactions, and only their exact joint outcome may publish both pointers, lifecycle state-v2, lineage, and fold attribution in one manifest CAS. Recovery-v11 is historical only and is refused under v9 and later because it lacks the token participant and complete state-v2 outcome. Current schema v11 replaces the v10 boolean with protocol-v2 profile state: `ENABLED` carries an active fold delegation, `DISABLING` carries a drain-only continuation plan, `RETIRED` is receipt/cut-bound and fail-closed, and every state carries bounded receipt-chain authority. The ambient flip is gone: only a checked offline cluster-apply capability bound to the persisted state lock, exact graph/declaration/profile revision, operation, and actor may change it; enabled-profile ordinary content writes require the exact checked cluster serving runtime. Direct `--store` and embedded writers cannot manufacture either authority. Recovery schema v13 is the current ceiling and emits only `StreamProfileChange`: it fixes one exact token-ledger `ProfileManagementReceipt` transaction, and the terminal manifest CAS selects that token witness together with the next profile. Unknown or later lifecycle discriminators fail closed, and v12 is not reinterpreted. Profile apply enforces `stream_manage` under both the currently applied and desired graph policies through the cluster-state CAS. Enabled-profile Mutation/Load/delete require the exact checked served runtime; BranchMerge remains closed under both `ENABLED` and `DISABLING` even with that runtime. Checked offline apply may finish disable when no lifecycle rows exist or every existing lane is already `SEALED`, but cannot drain a non-`SEALED` lane. There is still no public ingress, production enrollment, drain, claim, lifecycle correction, exclusive-cut status, generation GC, fresh read, or streaming SDK/HTTP/CLI/OpenAPI surface. The remaining lifecycle family was not registered under v11/v13 and therefore requires another strict graph/recovery strand. | [RFC-026](../rfcs/0026-memwal-streaming-ingest.md), [writes.md](writes.md) |
...
- **Private B2 token/fold core and B2a retain-all profile are implemented;
  ...
  design. Explicit production enrollment, lifecycle-management receipts,
  strict correction, exclusive-cut physical status, row admission, and
  transport parity remain required before a public profile. The
...
- **Branch-merge manifest history amplification:** coordinator open/full-refresh
  ...
  therefore research-blocked and production remains on the current v11 journal
