...
| Schema apply concurrency | Acquires the touched-existing-table write queues (`rewritten ∪ rename-source ∪ dropped`) **before** the snapshot — same queue-before-read order as `optimize` — and publishes with a per-table expected-version CAS. The old graph-global "write lease" check is gone: the global manifest version advancing for a table the apply does **not** touch no longer false-fails it (disjoint manifest row, publisher rebases). New concurrent writers refuse while the apply holds `__schema_apply_lock__` (`ensure_schema_apply_idle` / `ensure_schema_apply_not_locked`); a writer that was in flight before the lock is serialized on the per-table queue (touched tables); the per-table CAS is the loud cross-process backstop (`bug 4 / iss-schema-apply-optimize-occ`) | [writes.md](writes.md) |
...
- **Recovery is serialized against live writers in-process only:** the
  ...
  Narrowed by bug 4: a schema apply's *publish* is now fenced by a per-table
  expected-version CAS, so a foreign-process optimize that moved one of the
  apply's touched tables surfaces as a loud `ExpectedVersionMismatch` rather
  than a silent clobber. The residual here is specifically the recovery
  heal / schema-staging promotion path, not the apply's own publish.
