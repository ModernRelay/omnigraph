...
- **Private B2 token/fold core and B2a retain-all profile are implemented;
  ...
  Hidden F4 caller-shaped ingress/prepare and format-neutral F5a automatic
  `OPEN`-lane folding are implemented behind doc-hidden checked-runtime seams.
  ...
  Ordinary automatic `DRAINING` continuation and public driver status remain
  future. No active path produces `AuthorityBlock`, so its repair remains
