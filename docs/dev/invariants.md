...
- **Private B2 token/fold core and B2a retain-all profile are implemented;
  ...
  and terminal-scan harness; the F6b remainder still owns covered/reconciled
  token evidence and threshold, isolated dead-letter encoding/materialization
  byte and peak-RSS cap evidence, bounded export transport, public status, and
