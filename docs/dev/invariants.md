...
8. **Schema identity survives renames.** Accepted type and property identities
   ...
   lifecycle proofs without a token receipt or caller operation ID. Current
   v16 raises the ceiling to recovery-v18 for the complete private physical
   rebind owner: an exact `SEALED` lane moves to a fresh empty binding scope
   and remains `SEALED`. Older incomplete resume/maintenance/rebind scaffolds
...
- **Private B2 token/fold core and B2a retain-all profile are implemented;
  ...
  checked `SEALED` Optimize capability; current v16 adds the recovery-v18
  private physical-rebind owner. Public/production rebind remains inactive.
  Production enrollment, quiesce/resume/abort, strict
  correction/retirement, exclusive-cut physical status,
...
- **Branch-merge manifest history amplification:** coordinator open/full-refresh
  ...
  therefore research-blocked and the current development format remains on the v16 journal
