...
8. **Schema identity survives renames.** Accepted type and property identities
   ...
   checked-runtime, canonical-main `SEALED` EnsureIndices capability. Current
   v15 raises it to recovery-v17 for the distinct checked-runtime `SEALED`
   ...
   lifecycle proofs without a token receipt or caller operation ID; older
   incomplete resume/maintenance/rebind scaffolds remain frozen and
   fail-closed.
   See
...
- **Private B2 token/fold core and B2a retain-all profile are implemented;
  ...
  `SEALED` EnsureIndices capability; current v15 adds the distinct
  recovery-v17 checked `SEALED` Optimize capability. Rebind remains inactive.
...
- **Branch-merge manifest history amplification:** coordinator open/full-refresh
  ...
  therefore research-blocked and the current development format remains on the v15 journal
