...
- **Private B2 token/fold core and B2a retain-all profile are implemented;
  ...
  Public enrollment and general resume/abort and lifecycle control remain
  ...
  bounded/managed profile, not this one. F7a's graph-native served row path and
  F7b's graph-redacted checked status path are active; public lane enrollment
  and lifecycle/maintenance CLI/HTTP/OpenAPI surfaces remain inactive. The stopped/offline retirement
