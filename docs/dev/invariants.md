...
8. **Schema identity survives renames.** Accepted type and property identities
   ...
   and remains `SEALED`. Current v17 raises the ceiling to recovery-v19 for the
   ...
   selected frozen branch member beside that root receipt. Older incomplete
   resume/maintenance/retirement/rebind scaffolds remain frozen and
   fail-closed; authority correction remains inactive.
   See
...
- **Private B2 token/fold core and B2a retain-all profile are implemented;
  ...
  physical-rebind owner; current v17 adds recovery-v19 stopped/offline terminal
  authority retirement and export carrying the root receipt plus a closed
  selected-branch-member witness. Public/production rebind
  remains inactive.
  Production enrollment, quiesce/resume/abort, strict
  correction, exclusive-cut physical status,
  ...
  bounded/managed profile, not this one. CLI/HTTP/OpenAPI streaming surfaces
  remain inactive.
...
- **Branch-merge manifest history amplification:** coordinator open/full-refresh
  ...
  therefore research-blocked and the current development format remains on the v17 journal
