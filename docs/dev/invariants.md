...
8. **Schema identity survives renames.** Accepted type and property identities
   ...
   Current v13 raises the ceiling to recovery-v15 for the complete hidden
   resume/guarded drain-abort owner; v14's incomplete resume scaffold remains
...
| Streaming ingest | RFC-026 adopts Lance MemWAL as the strategic substrate and remains Draft; public row streaming is inactive. Historical schema v7 introduced recoverable main-only empty enrollment, v8/config-v2/recovery-v11 added the bounded B1 worker, and v9/config-v3/state-v2/recovery-v12 activated the private compare-and-chain/token-fold core. V10 added the graph-global profile singleton; v11 replaced it with checked protocol-v2 profile authority and exact recovery-v13 `StreamProfileChange`. V12 replaces inline lifecycle history with lifecycle-v3 fixed-size binding/management/claim ledger heads and activates recovery-v14 `StreamEnrollmentV2`, `StreamClaim`, `StreamFoldV2`, `StreamDrainFold`, and `StreamLifecycleReceipt`. Current v13 activates recovery-v15 `StreamResume`: a revision-fenced `SEALED → OPEN` resume or guarded `DRAINING → OPEN` abort owns its higher-epoch physical claim and terminal claim/management receipts before the sole `OPEN` publication. V14's resume scaffold and its correction/retirement/maintenance/rebind discriminators remain frozen and fail closed. Every cold opener and fold first publishes an exact claim whose receipt chain authenticates the bounded WAL suffix and full current-generation LWW projection. Exclusive admission then supports restartable hidden `OPEN → DRAINING → SEALED` quiescence for empty and non-empty lanes; restart after a terminal claim reclaims with a higher epoch, while restart after physical seal reuses the exact flushed cut. Base and token effects publish only as one exact joint outcome with lifecycle, lineage, and attribution. Historical v10 enrollment, v12 lifecycle-v2 fold, and v14 resume payloads retain their old wire meaning and are refused. Admission recaptures durable authority after waiting; claims, folds, and resume additionally hold profile-shared then schema → branch → stream-token → table gates across final recapture, effect, and publication. The manifest-selected `_stream_tokens.lance` witness—not its raw HEAD—is authority. Enabled-profile ordinary content writes require exact checked cluster runtime authority; BranchMerge remains closed under `ENABLED` and `DISABLING`. There is still no public ingress, production enrollment/quiesce/resume/abort, correction/retirement, maintenance/rebind, generation GC, fresh read, or streaming SDK/HTTP/CLI/OpenAPI surface. | [RFC-026](../rfcs/0026-memwal-streaming-ingest.md), [writes.md](writes.md) |
...
- **Private B2 token/fold core and B2a retain-all profile are implemented;
  ...
  restartable empty/non-empty quiescence; current v13 adds the recovery-v15
  private resume and guarded drain-abort owner. Production enrollment,
  quiesce/resume/abort, strict correction/retirement, exclusive-cut physical status,
...
- **Branch-merge manifest history amplification:** coordinator open/full-refresh
  ...
  therefore research-blocked and the current development format remains on the v13 journal
