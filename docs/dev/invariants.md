...
| Streaming ingest | RFC-026 adopts Lance MemWAL as the strategic substrate and remains Draft; public row streaming is inactive. Historical schema v7 introduced recoverable main-only empty enrollment, v8/config-v2/recovery-v11 added the bounded B1 worker, and v9/config-v3/state-v2/recovery-v12 activated the private compare-and-chain/token-fold core. V10 added the graph-global profile singleton; v11 replaced it with checked protocol-v2 profile authority and exact recovery-v13 `StreamProfileChange`. V12 replaces inline lifecycle history with lifecycle-v3 fixed-size binding/management/claim ledger heads and activates recovery-v14 `StreamEnrollmentV2`, `StreamClaim`, `StreamFoldV2`, `StreamDrainFold`, and `StreamLifecycleReceipt`. V13 activates recovery-v15 `StreamResume`; v14–v18 add the distinct checked maintenance, rebind, retirement, and exact DataBlock-correction owners through recovery-v20. Current v19/token-schema-v3/recovery-v21 activates the hidden deterministic mixed/all-diverted terminal fold: valid winners publish normally, every losing candidate enters one bounded canonical object, and one current `DEAD_LETTERED` token per losing key preserves exact retry and predecessor sequencing. An ordinary successor can restore `PRESENT`. The stopped/offline cluster path lists current terminal authority and exports its descriptor-verified payload without prefix listing or a second inventory; ordinary export remains blocked by current `WITHDRAWN | DEAD_LETTERED`, while same-format retirement now covers either disposition. Historical recovery-v19 retirement and recovery-v20 correction retain their exact meanings. Every cold opener/fold still publishes exact claim authority, base and token effects remain one manifest-visible outcome with lifecycle/lineage/attribution, and enabled-profile writers remain checked-runtime fenced. F4/F5a/F5b0 provide the hidden caller path, automatic folding, goal-`SEALED` continuation, and deterministic offline disable loop. F6a adds one hidden in-process composed acceptance plus a typed failpoints-only process-local advisory driver snapshot; pending triggers are not a durable backlog, stopped run state is not offline authority, and public durable `StreamStatus` remains manifest-only. Checked offline disable is the sole supported production quiescence owner. There is still no public ingress/enrollment, general resume/abort/rebind, public driver health/backlog status, generation GC, fresh read, or streaming SDK/HTTP/OpenAPI surface. The narrow cluster/offline controls are profile apply, retirement, `stream block show|correct`, and `stream dead-letter list|export`. F6b still owns measurements, OS-process forced-termination evidence, full node+edge/fairness, long-history lookup/RSS/latency, and the remaining guardrail/composition matrix. No active path produces reserved `AuthorityBlock`, so its repair remains fail-closed. | [RFC-026](../rfcs/0026-memwal-streaming-ingest.md), [writes.md](writes.md) |
...
- **Private B2 token/fold core and B2a retain-all profile are implemented;
  ...
  Public driver status remains future. No active path produces
  `AuthorityBlock`, so its repair remains fail-closed until a producer and
  evidence grammar exist. F6b still owns the dead-letter object/RSS and
  selected-token lookup measurements plus the full guardrail matrix; the v19
  terminal protocol itself is active behind the hidden row path.
