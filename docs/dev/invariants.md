...
8. **Schema identity survives renames.** Accepted type and property identities
   ...
   resume/guarded drain-abort owner. Current v14 raises it to recovery-v16 for
   the checked-runtime, canonical-main `SEALED` EnsureIndices capability. That
   capability atomically refreshes productive table pointers and lifecycle
   proofs without a token receipt or caller operation ID; v14's older
...
- **Private B2 token/fold core and B2a retain-all profile are implemented;
  ...
  resume and guarded drain-abort owner. Current v14 adds only the
  recovery-v16 checked `SEALED` EnsureIndices capability; Optimize and rebind
  remain inactive. Production enrollment, quiesce/resume/abort, strict
...
- **Branch-merge manifest history amplification:** coordinator open/full-refresh
  ...
  therefore research-blocked and the current development format remains on the v14 journal
