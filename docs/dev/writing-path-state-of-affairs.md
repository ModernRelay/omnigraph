...
private. Public lane enrollment, general lifecycle resume/abort/rebind, and
maintenance/status transports remain inactive;
retirement,
...
boundary, and engine-internal checked operational-status cut exist, but there
is still no `@stream` syntax, public lane-enrollment/fold API, enrolled-lane
drain/resume/correction surface, operational-status transport, or fresh-read
mode.
