...
**Surveyed:** OmniGraph 0.9.0 development, internal manifest schema v15,
Lance 9.0.0
...
capability, and the distinct v15/recovery-v17 checked `SEALED` Optimize bridge.
...
Current v15 adds the separate recovery-v17 Optimize bridge for internally
committing compaction/index-maintenance effects, exact achieved HEADs, and the
same atomic lifecycle-proof refresh. Older incomplete
resume/maintenance/rebind scaffolds remain frozen.
...
retirement, physical rebind, and public maintenance surfaces
remain inactive even though the
hidden one-lane lifecycle core now exists.
...
`SEALED` EnsureIndices capability; current v15 adds the distinct recovery-v17
`SEALED` Optimize capability. The next work is physical rebind,
correction/retirement, and explicit
...
| Maintenance | EnsureIndices stages exact missing-index transactions. Its private checked-runtime recovery-v16 path can refresh an enrolled table only at exact `SEALED`; ambient calls remain refused. Optimize coordinates graph-wide compaction/index work under one bounded recovery envelope; its private recovery-v17 path applies the same checked-runtime, exact-`SEALED` pointer/proof refresh while ambient calls remain refused for enrolled tables. Periodic optimize compacts `__manifest`, but unmaintained history-dependent paths are not globally flat. |
...
| [023 — Key-conflict fencing](../rfcs/0023-key-conflict-fencing.md) | **Implemented** | Internal schema v6 introduced exact-`id` PK metadata, closed keyed routing, typed conflicts, bounded replay, rebuild/refusal, and accepted performance evidence; v15 preserves that contract. |
...
| [026 — MemWAL streaming ingest](../rfcs/0026-memwal-streaming-ingest.md) | **Draft; private B2 token/fold, retain-all, profile-authority, hidden lifecycle, and narrow SEALED maintenance tranches implemented; no public row path** | Schema v9 introduced config-v3/state-v2 token authority plus recovery-v12 exact base+token fold. V11/profile-v2 adds checked profile ownership, resumable `DISABLING`, and exact recovery-v13 `StreamProfileChange`. V12/lifecycle-v3 + recovery-v14 adds immutable enrollment/binding receipts, recovery-covered claims, exact ordinary/drain folds, and restartable hidden empty/non-empty quiescence. V13/recovery-v15 adds private resume and guarded drain-abort; v14/recovery-v16 adds capability-only SEALED EnsureIndices; current v15/recovery-v17 adds capability-only SEALED Optimize. Historical recovery-v10/v12/v14-resume/maintenance payloads are refused. Gate R0's no-go still prohibits a finite storage promise. Production enrollment/quiesce/resume/abort, correction/retirement, exclusive-cut physical status, physical rebind, public row admission, and CLI/HTTP/OpenAPI surfaces remain inactive; B2b managed reclamation is optional future work. |
...
| [028 — Stable schema identity](../rfcs/0028-stable-schema-identity.md) | **Implemented** | Rename-stable IDs, table incarnation, identity-derived paths, schema/recovery integration, and strict rebuild activation were introduced in v5 and remain active in v15. |
