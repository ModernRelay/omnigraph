...
resume/abort/rebind, and maintenance/status transports remain inactive;
retirement,
...
above are active. The graph-scoped Cedar vocabulary, public manifest-only
status, checked profile-control boundary, and engine-internal checked
operational-status cut exist, but there is still no `@stream`
...
physical/driver-status transports remain inactive. F6b5 is the narrow
transport exception: the existing served HTTP/remote-CLI/OpenAPI export route
