...
| Crate | Path | Style |
|---|---|---|
| `omnigraph-db` (engine; dir `crates/omnigraph`) | `crates/omnigraph/tests/` | Integration tests (one file per behavior area — see the table below), fixture-driven, share `tests/helpers/mod.rs`. The `compiler`, `policy`, and `storage_backend` modules (former sibling crates, merged for the 0.9.1 single-published-crate restructure) keep their in-source `#[cfg(test)]` suites — see the compiler row below |
...
| `omnigraph::compiler` (module of `omnigraph-db`) | in-source `#[cfg(test)] mod tests` under `crates/omnigraph/src/compiler/` | Parser, type-checker, IR lowering, lint. Schema parser and SchemaIR validation tests both reject the five exact Lance virtual system-column property names while preserving near-miss identifiers |
...
| `forbidden_apis.rs` | Defense-in-depth source guard for the sealed storage boundary: raw Lance and coordinator surfaces stay crate-private, public snapshots expose safe scanners, graph writers use approved staged adapters, and retired WAL/stream symbols and durable call sites remain absent. |
| `lance_surface_guards.rs` | Compile/runtime pins for the Lance APIs OmniGraph depends on: shared object-store registries with isolated metadata caches, uncommitted index shapes, blob compaction, branch reclaim, exact transaction/head witnesses, PK conflict filters, cleanup refs, virtual system-column reservations, the exact-end handle requirement for historical delta row images, and stable row/version columns on OmniGraph-created graph tables. Run first on every Lance bump. |
...
| `lifecycle.rs` | Graph lifecycle and schema state, including the v6 creation invariant that every fresh node/edge table declares exactly physical non-null `id` as Lance's unenforced primary key. |
| `point_in_time.rs` | Snapshots, time travel (`snapshot_at_version`, `entity_at`) |
| `changes.rs` | `diff_between` / `diff_commits`, including immutable-identity table pairing: pure renames stay empty while drop/re-add under one alias remains two table lifetimes. The production net diff consumes the same identity-keyed table-interval derivation reserved for later per-commit CDC row walking and pins graph-visible table-key ordering across changed lifetimes; it does not yet claim complete CDC block/cursor ordering. |
...
- `cargo test -p omnigraph-db --test s3_storage` (lifecycle/branching + the e_tag-present CSR topology cache-key reuse test — the path local FS can't reach since its e_tag is `None`)
- `cargo test -p omnigraph-db --test lance_surface_guards public_physical_ref_token_rejects_s3_same_version_aba -- --exact` (RFC-024's public current-HEAD witness across unchanged reopen plus main/named same-version ABA; the workflow additionally rejects a zero-test/vacuous match)
...
- `cargo test -p omnigraph-db --features failpoints --test failpoints s3_` (recovery-sidecar lifecycle on a real bucket)
...
it on demand with
`OMNIGRAPH_S3_TEST_BUCKET=… cargo test -p omnigraph-db --test durable_head_lookup_cost s3_durable_head_lookup_matrix_is_correct_and_observable -- --exact --nocapture`.

RFC-025's S3 **cost** matrix is likewise on demand and was not run for the
2026-07-17 local no-go decision:
`OMNIGRAPH_S3_TEST_BUCKET=… cargo test -p omnigraph-db --test checkpoint_retention_cost s3_checkpoint_retention_matrix_is_exact_and_records_the_current_no_go -- --exact --nocapture`.
...
- `crates/omnigraph/benches/scenarios.rs` — the **scenario benchmark harness**: a
  ...
  `cargo bench -p omnigraph-db --bench scenarios -- --scenario
