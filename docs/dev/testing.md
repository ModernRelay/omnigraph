...
| `lifecycle.rs` | Graph lifecycle and schema state, including the v6-origin creation invariant—preserved through current v14—that every fresh node/edge table declares exactly physical `id` as its Lance unenforced PK |
...
cells before the capability broadens. Optimize and physical rebind remain
separate inactive slices.
...
lives in `memwal_stream.rs` and the v14/v15 in-source suites below. In-source
...
- in-source `stream_request` unit tests own the transport-neutral request
  ...
  bounded current-binding ancestry validation; physical rebind remains absent;
...
mutation. The current v14 hidden lifecycle owners are `memwal_stream.rs` plus
the v14/v15/v16 in-source recovery/lifecycle suites described above; those tests still
...
by the old binary. The current required gate is the final-v13 → v14 seam below.
...
The current immediate-predecessor seam is `OMNIGRAPH_V13_BIN`. It mints a
...
`OMNIGRAPH_V11_BIN`, `OMNIGRAPH_V12_BIN`, and `OMNIGRAPH_V13_BIN`, fails loudly rather than making
the proof vacuous.
