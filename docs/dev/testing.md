...
| `export.rs` | NDJSON streaming export filters; its Blob fixture pins exact null, valid-empty-leading-fragment, non-empty neighbour, and preserved external-reference spellings without opening an unavailable target, plus rebuild fidelity, then performs a later `LoadMode::Append` strict insert into the populated current-format table (preserving the v6 PK contract) and verifies exact bytes and exact-`id` PK metadata afterward. `export_jsonl_round_trips_branch_snapshot` separately exports `main` and a named feature branch, rebuilds each into a main-only graph, and proves independent identity domains plus disjoint, self-contained histories |
