...
| `traversal_indexed.rs` | BTREE-indexed Expand (`execute_expand_indexed`) forced via `OMNIGRAPH_TRAVERSAL_MODE`, asserted semantically equal to the CSR path; own binary, all `#[serial]` so env writes never race (the `ModeGuard` RAII helper clears the override even on a panic). Also home to the CSR topology-build cost guards (which force the same env): `fresh_branch_traversal_reuses_main_graph_index` (A1 — a lazy-fork branch reuses main's cached CSR index, 0 rebuilds via the `graph_build_count` probe) and `single_edge_query_builds_only_referenced_edge` (A2 — a one-edge query builds only that edge via the `graph_edges_built` probe). |
