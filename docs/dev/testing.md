...
| `lifecycle.rs` | Graph lifecycle and schema state, including the v6-origin creation invariant—preserved by v7—that every fresh node/edge table declares exactly physical `id` as its Lance unenforced PK |
...
## Cross-version upgrade (genuine old binaries → v7)
...
case uses 0.8.1. Both prove current-v7 refusal, export/init/load into a different
v7 root, row/vector fidelity, and exact-`id` PK metadata on every rebuilt graph
...
proves v7 refuses it with the 0.9.x rebuild guidance, exports with v5, rebuilds
under v7, checks row/vector/blob fidelity, exact blob bytes, and exact-`id` PK
metadata, then proves the v5 binary refuses the v7 root. The same cell injects
a duplicate logical ID into the v5 export: v7 rejects the load atomically,
...
RFC-026 Phase A adds the immediate-predecessor seam
`OMNIGRAPH_V6_BIN`. It mints a genuine internal-v6 graph, proves v7 refuses it
before serving, exports with v6, rebuilds into a different v7 root, verifies
row/vector fidelity and exact-`id` PK metadata, and proves the v6 binary refuses
the v7 root. This is the format-boundary evidence that a stamp-rewind test
cannot supply.

```bash
OMNIGRAPH_V6_BIN=/path/to/final-v6/omnigraph \
  cargo test -p omnigraph-cli --test crossversion_upgrade --locked \
  current_v7_refuses_and_rebuilds_genuine_v6_and_v6_refuses_v7 -- --nocapture
```
