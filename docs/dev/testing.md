...
| `merge_net_zero.rs` | #473 net-zero merge owner: lineage-only and mixed real-plus-net-zero deltas into main and named targets, with exact manifest movement, table-pointer suppression, content, and idempotence assertions. Adopt equality regressions pin null versus empty, U+001F framing, legal `_row_id`, and managed Blob file qualification. |
...
| `failpoints.rs` | Feature-gated crash-window coverage for ordinary recovery-v9 writers: Mutation/Load, BranchMerge, SchemaApply, EnsureIndices, Optimize, native branch controls, first-touch refs, manifest publication, recovery discovery, cleanup, and S3 twins. Effects remain invisible until the one manifest publication and ambiguous outcomes remain recovery-owned. The net-zero merge cell proves a pre-confirmation crash compensates the real sibling delta, preserves source and target rows, leaves the suppressed table untouched, and remains exactly retryable. Conditional mutation cells park effectful update/delete and zero-match attempts before their authoritative gate, advance the branch, and require terminal `PreconditionFailed` before loser effects; a live-read refresh cell injects failure between replacement state and inherited-lineage decoding and proves the next read retries a coherent branch-incarnation view. RFC-033 additionally parks a live named-branch Blob read after graph capture, replaces that branch at the same physical path/version, and proves the reader fails the cold incarnation proof instead of returning replacement bytes. RFC-030's `CHANGE_FEED_POST_CAPTURE` cell is the change-feed twin for the first ABA window: it parks a poll after cut capture, delete/recreates the polled branch at the same version, and proves `commit_snapshot`'s incarnation re-prove fails closed rather than emitting the replacement branch's rows under the old commit's label. `change_feed_poll_refuses_delete_recreate_aba_before_table_open` (`CHANGE_FEED_PRE_TABLE_OPEN`) is the second-window twin: it isolates a single commit, parks between the head proof and the per-table open, delete/recreates the branch at the same path/version, and proves `open_at_entry_verified`'s e_tag incarnation check fails closed instead of reading the replacement branch's rows. `change_feed_poll_refuses_table_open_aba_without_etag_witness` combines that park with the `CHANGE_FEED_SKIP_ETAG_WITNESS` behavior seam (simulating an e_tag-less store) and proves the LOGICAL post-open witness — `reprove_named_branch_heads`'s fresh manifest reopen comparing `graph_head` at the pinned version — refuses the replacement alone; the e_tag comparison is defense-in-depth, not the load-bearing witness. |
