...
| `lifecycle.rs` | Graph lifecycle and schema state, including the v6-origin creation invariant—preserved through current v16—that every fresh node/edge table declares exactly physical `id` as Lance's unenforced PK |
...
mutation. The current v16 hidden lifecycle owners are `memwal_stream.rs` plus
the v14/v15/v16/v17/v18 in-source recovery/lifecycle suites described above; those tests still
...
by the old binary. The current required gate is the final-v15 → v16 seam below.
...
The current immediate-predecessor seam is `OMNIGRAPH_V15_BIN`. It builds the
final v15 binary from immutable merge
...
`OMNIGRAPH_V14_BIN`, and `OMNIGRAPH_V15_BIN`, fails loudly rather than making
the proof vacuous.
