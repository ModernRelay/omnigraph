...
| `omnigraph-dst` | `crates/omnigraph-dst/tests/scenarios.rs` (+ `tests/torn_init.rs`) | Deterministic simulation: seeded universes drive a real `Omnigraph` through the production write path against a differential model, durability, OCC-uniqueness, and replay oracles; crash-window enumeration over the engine's failpoint catalog; concurrent universes under a seeded seam scheduler. Needs `--cfg tokio_unstable` — run cargo from `crates/omnigraph-dst/` (its crate-local `.cargo/config.toml` sets the flag) or set `RUSTFLAGS` yourself; every test file is `#![cfg(tokio_unstable)]`-gated and the crate compiles empty without it, so the default workspace gate is unaffected. `#[ignore]`d tests are long-running fleet/hunt instruments driven by the DST workflows, not correctness cells. |
