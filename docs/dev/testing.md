...
| `forbidden_apis.rs` | Defense-in-depth source guard for the sealed storage boundary, plus `write_reprepare_paths_do_not_run_full_recovery` (mutate/load reprepare through `refresh_coordinator_only`, never the rollback-capable sweep — a source guard because `refresh()` can gain semantics in a change that touches neither file): raw Lance and coordinator surfaces stay crate-private, public snapshots expose safe scanners, graph writers use approved staged adapters, and retired WAL/stream symbols and durable call sites remain absent. |
...
| `warm_read_cost.rs` | Cost-budget tests for the warm read/control path (query-latency work), measured at the object-store boundary with Lance `IOTracker` (the LanceDB IO-counted pattern): a warm same-branch read does 0 manifest opens, 1 version probe, validates the schema once (Fix 1 / finding A / Fix 2 at commit-history depth); a cold other-branch resolution derives snapshot state and lineage from one coherent manifest open/scan; native branch create and create-from each use one post-gate open/scan, while delete uses one target capture plus one native-ref opener and only one row scan; stale same-branch reads perform exactly 2 probes and refresh manifest-only; recreated non-main branches with the same Lance version refresh by incarnation; recreated branch-owned table handles are distinguished by table e_tag or refresh-time cache clearing; recreated traversal topology is protected by per-edge-table e_tag in the graph-index cache key or refresh-time cache clearing; a warm *repeat* read does 0 table opens via the held-handle cache and a write re-opens only the changed table at its new version/e_tag (Fix 3/6A). A no-sidecar `refresh()` costs exactly one manifest open and one scan (`refresh_without_sidecars_uses_one_manifest_open`) — the served-graph recovery primitive must not pay a second full scan per call, nor follow commit depth. Also the CSR topology-build cost guards: `fresh_branch_traversal_reuses_main_graph_index` (A1 — a lazy-fork branch reuses main's cached CSR index, 0 rebuilds via `graph_build_count`) and `single_edge_query_builds_only_referenced_edge` (A2 — a one-edge query builds only that edge via `graph_edges_built`); both force CSR via the scoped `with_traversal_mode` seam, so they need no `#[serial]`. See "Cost-budget tests" below. |
...
| `failpoints.rs` | Feature-gated crash-window coverage for ordinary recovery-v9 writers: Mutation/Load, BranchMerge, SchemaApply, EnsureIndices, Optimize, native branch controls, first-touch refs, manifest publication, recovery discovery, cleanup, and S3 twins. Effects remain invisible until the one manifest publication and ambiguous outcomes remain recovery-owned. Live-refresh recovery is covered here too: `failed_refresh_publishes_nothing_and_still_converges` proves the prepare-then-publish shape leaves the handle's schema projection untouched when preparation fails and converges on retry (the coordinator half is not black-box observable, since reads self-refresh), and `read_only_refresh_never_runs_recovery` proves a read-only handle neither consumes nor rewrites a recovery sidecar while a read-write handle still does. |
