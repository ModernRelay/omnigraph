...
| `warm_read_cost.rs` | Cost-budget tests for the warm read path (query-latency work), measured at the object-store boundary with Lance `IOTracker` (the LanceDB IO-counted pattern): a warm same-branch read does 0 manifest opens, 0 commit-graph opens, 1 version probe, validates the schema once (Fix 1 / finding A / Fix 2 at commit-history depth); stale same-branch reads perform exactly 2 probes and refresh manifest-only; recreated non-main branches with the same Lance version refresh by incarnation; recreated branch-owned table handles are distinguished by table e_tag or refresh-time cache clearing; recreated traversal topology is protected by edge-table physical identity (e_tag) or refresh-time cache clearing; a warm *repeat* read does 0 table opens via the held-handle cache and a write re-opens only the changed table at its new version/e_tag (Fix 3/6A); a fresh lazy-fork branch reuses main's CSR topology index with 0 rebuilds, and a query referencing one edge builds only that edge (not all catalog edges) — both `#[serial]` + `OMNIGRAPH_TRAVERSAL_MODE=csr` (A1/A2). See "Cost-budget tests" below |
