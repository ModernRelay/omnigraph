...
| File | Covers |
|---|---|
| `end_to_end.rs` | Full init → load → query/mutate flow; Blob coverage includes `blob_read_after_mutation_insert`, where a handle opened before another handle's commit must freshness-probe and read the newly committed Blob, `blob_load_external_file_uri`, which proves an explicitly allowed Overwrite persists the exact normalized reference proven by preflight (including a `file://` symlink that is retargeted outside the base after admission) while two normalized-equivalent keyed inputs produce one metadata probe, one batch-bounded payload read, and managed copies, and `update_with_blob_mid_schema_does_not_panic`, which carries a valid empty Blob through a scalar mutation rewrite and proves it remains non-null and empty. Blob-bearing queries project only ordinary properties; Blob-valued `.gq` reads are rejected by compiler `T24`. |
