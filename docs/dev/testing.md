...
| `memwal_stream.rs` | Feature-gated RFC-026 private B1 mechanics, implemented private B2-common token/fold behavior, and B2a provider-failure evidence. B1 owns bounded put/ack/replay/fold, authority, cancellation, and manifest-only visibility. B2-common owns compare-and-chain/idempotency conflicts, same-generation overlays, stale-authority recapture after shared admission, the byte-for-byte recovery-v12 exact base+token fold, crash completion, and durable attribution. B2a injects a recording/failing store at the real Lance table-store boundary and covers effect-free cold-claim failure, post-invocation WAL ambiguity, complete post-cut unreferenced generation output, and partial generation output after the data object. Exact local and configured-RustFS cells prove typed `Lance` / `AckUnknown` / `RecoveryRequired` outcomes, blocked fresh progress, one fresh authoritative retry root, retained non-authoritative residue, and zero access below the orphan root through retry and cold reopen. Parent shard discovery may observe the common prefix; it may not descend into or adopt the subtree. V11 profile-authority behavior belongs to manifest/control-authority/cluster tests, not this private row/fold fixture; no test here implies public ingress or full lifecycle activation. |
...
| `lifecycle.rs` | Graph lifecycle and schema state, including the v6-origin creation invariant—preserved through current v11—that every fresh node/edge table declares exactly physical `id` as its Lance unenforced PK |
...
### RFC-026 private B2-common coverage ownership (implemented)

The private B2-common slice extends the existing owners rather than creating a
parallel test stack. `memwal_stream.rs` owns compare-and-chain retries and
conflicts, same-generation token overlays, stale-authority recapture, exact
base+token fold, and the byte-for-byte recovery-v12 crash boundaries. Unit tests in
`db/manifest/stream_token.rs` own canonical token/payload derivation and corrupt
base/token refusal; `db/manifest/recovery.rs` owns the v12 two-participant
effect matrix and confirmation. Manifest tests own the graph-global selected
token pointer and fold-attribution publication. `lifecycle.rs` proves hidden
trusted metadata and the MemWAL system index stay out of public reflection,
while `forbidden_apis.rs` proves no SDK row or lifecycle-control and no
CLI/HTTP/OpenAPI ingest side door was introduced; it classifies the embedded
read-only `stream_status` surface separately.
...
cargo test -p omnigraph-engine stream_fold_v12
...
`cluster apply --confirm-stream-offline` control surface. No test in this slice
activates public ingress, production enrollment, drain, claim, or lifecycle
mutation; those require another strict graph/recovery strand.
...
## Cross-version upgrade (genuine old binaries → v9)
...
case uses 0.8.1. Both prove current-v9 refusal, export/init/load into a different
v9 root, row/vector fidelity, and exact-`id` PK metadata on every rebuilt graph
...
The current immediate-predecessor seam is `OMNIGRAPH_V10_BIN`. It mints a
genuine final-v10 graph with the matching 0.10.0-dev source build, proves v11
refuses it with source-build/export guidance, exports with v10, rebuilds a
different v11 root, and proves row/vector/blob fidelity plus exact-`id` PK
metadata. The old v10 binary must refuse the v11 root. This is format evidence
only: v10 had no production enrollment or public row-ingest path, and the
rebuild deliberately does not transfer private lifecycle, WAL, token, or
receipt authority. The fixture is clean, disabled, and unenrolled; it does not
claim that v10's ordinary exporter detects an artificial test-only pending-WAL
image. Such an image is outside the supported v10 rebuild contract and must be
quarantined rather than treated as transferred. Run it locally with:
...
`OMNIGRAPH_V8_BIN`, `OMNIGRAPH_V9_BIN`, and `OMNIGRAPH_V10_BIN`, fails loudly
