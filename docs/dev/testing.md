...
| `omnigraph-cluster` | mostly in-source `#[cfg(test)] mod tests`; `tests/failpoints.rs`; `tests/s3_cluster.rs` | Cluster config plus semantic policy-bundle and graph/server binding-kind validation, state CAS and locking, validate/plan/status/refresh/import, idempotent apply, graph create/schema/delete, policy binding, serving snapshots, crash windows, and object-storage lifecycle. |
