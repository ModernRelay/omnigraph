...
| `lifecycle.rs` | Graph lifecycle and schema state, including the v6-origin creation invariant—preserved through current v9—that every fresh node/edge table declares exactly physical `id` as its Lance unenforced PK |
...
(about 273 MiB) isolated fold RSS delta under one exclusive fold. Its 384-MiB
...
base+token fold, and the recovery-v12 crash boundaries. Unit tests in
...
while `forbidden_apis.rs` proves no SDK/CLI/HTTP/OpenAPI streaming side door was
introduced.
...
private because the common token, attribution, lifecycle, correction,
authorization, and product-parity contracts above are still inactive. A
