...
| `lifecycle.rs` | Graph lifecycle and schema state, including the v6-origin creation invariant—preserved through current v15—that every fresh node/edge table declares exactly physical `id` as its Lance unenforced PK |
...
Optimize slice under recovery-v17; physical rebind remains inactive.
...
physical-rebind evidence remain future work.
...
mutation. The current v15 hidden lifecycle owners are `memwal_stream.rs` plus
the v14/v15/v16/v17 in-source recovery/lifecycle suites described above; those tests still
...
by the old binary. The current required gate is the final-v14 → v15 seam below.
...
The current immediate-predecessor seam is `OMNIGRAPH_V14_BIN`. It builds the
...
`OMNIGRAPH_V14_BIN`, fails loudly rather than making
the proof vacuous.
