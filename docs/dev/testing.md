...
`memwal_stream.rs` owns hidden F4/F5a evidence. Public ingress and lifecycle
transports remain outside both slices.
...
recovery/lifecycle suites described above; those tests still do not activate a
...
create/verify. F6b still owns representative RSS/latency and authority-lookup
measurements plus the full guardrail matrix. No cell may assume a chunk chain,
...
Public durable `StreamStatus` remains manifest-only. F6a does not
cover OS-process forced termination, the full node+edge/fairness matrix,
measurements or long-history lookup/RSS/latency evidence, or maintenance/
rebind/resume composition; those remain F6b acceptance owners, and F7 stays
closed.
