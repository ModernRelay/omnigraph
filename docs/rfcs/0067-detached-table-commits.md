...
| Schema apply | exact Overwrite and create identities, Armed/EffectsConfirmed, rollback reclaims first-touch datasets | existing-table rewrites stage detached and promote; new-type creation is a linear v1 create at a fresh incarnation path; contract-file promotion follows the publication as idempotent authority-first work |
...
remain: schema apply and the full-text rebuild under detached staging; index
folding inside Optimize, which needs the upstream ask;
