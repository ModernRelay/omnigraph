...
│   │                · acquisition status, requested/observed counts, and
│   │                  nullable terminal failed-repetition index/stage/code
...
└── SEVEN PROTOCOL RULES · violation = invalid number (1-4, 7)
    ...
    ├── 3 reps + dispersion ······ bare means banned; one complete declared
    │                              cache condition per cell; small samples
    │                              labeled directional
...
- **Run record**: the immutable persisted description of one invocation: its run spec plus the measurements produced. One record holds the invocation's repetition rows; repetitions are never separate records and are never appended after publication. A `complete` record has exactly the requested number of fully verified rows. If repetition `k` fails after a non-empty verified prefix, a `censored` record may persist only rows `0..k`, stores `failed_repetition: k` plus a closed stable stage and canonical error-code token, and is permanently ineligible for performance claims; a failure at repetition zero publishes no record. A sample that reached the measured-operation settlement boundary but did not complete verification is not a row. The CLI remains failed when it publishes censored evidence. This prevents trimming or later mutation from shifting a cited percentile. The record is the citation target for every published number, but only records satisfying the complete claim policy may support such a number: acquisition must be complete and every required SUT/build proof, including effective-codegen proof, must be present. Acquisition completeness alone never sets global claim eligibility. The run spec remains the results table's ***natural key*** (a key formed from the data's own attributes rather than assigned), and `point_id` is its canonical persisted identifier: records with equal specs are directly comparable and form one series over time. Requested repetitions is invocation acquisition metadata and observed repetitions are the record's raw rows; neither changes point identity. One record is identified uniquely by an ***invocation id*** the harness mints when the invocation starts: globally unique by format (ULID-class), so identity never rests on clock resolution and the id alone may key derived surfaces; the id and the invocation timestamp (ordering only) are both persisted fields.
- **Cell**: one (run spec, SUT) pair's samples: every number in a cell shares every factor level and the subject ("series" is reserved for the spec grain: a point's records over time). Dispersion, cache-condition, and sample-count obligations attach at cell grain.
...
- **Cache condition**: the complete identity-bearing description of cache preparation before measurement: process lifecycle, engine preparation, OS page-cache condition, and the named warm-up program plus iteration count. These axes do not collapse into one `cold`/`warm` label. **Process-cold** is only a display label for a fresh process per repetition with preparation-only engine state, no declared warm-up program, and an uncontrolled OS page cache; normal engine open and scenario verification-preparation reads may already have populated engine and OS caches. **Warm** runs the named read-only program inside every repetition and measures with the same handle. Its `program-conditioned` OS page-cache level records a treatment—the named reads ran immediately before measurement—not a proof that every touched page remains resident or any statement about pages outside the read set. **Post-reopen** runs that program, drops the handle, and reopens before measurement; it claims a reopened handle, not cache invalidation. A fresh process proves nothing about OS page, device, object-store, or remote-service caches. A page-cache-cold level may be added only together with a named backend/platform control and a post-control eviction witness; absent both, the harness refuses the claim rather than approximating it. **Storage-cold** is neither supported nor representable: even witnessed OS page-cache eviction would not prove device, object-store, or remote-service cache state.
...
- **Extend the queryable result surface.** Canonical JSON records in the immutable-pointer archive are the source of truth. The shipped local projection rebuilds a fresh OmniGraph generation with one run node per invocation and one benchmark-point node per full `point_id`, exposes bounded fixed queries, and projects acquisition status and claim eligibility. Multi-user policy, remote publication, richer stored comparisons, and time-travel citation UX remain later work; the projection stays disposable and rebuildable from the archive.
