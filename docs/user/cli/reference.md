...
| `mutate <name>` (alias: `change`) | run a mutation query; same catalog (by-name, served-only, verb asserts mutation) / ad-hoc (`--query`/`-e`) lanes as `query`. `--if-commit <id>` makes the write conditional (compare-and-swap): it runs only if the branch head commit (from `commit list`) still equals `<id>`; a lost race exits with code 4 and, with `--json`, the structured `precondition_failure` body — re-read and decide again. `change` is the deprecated previous name (one-line stderr warning) |
