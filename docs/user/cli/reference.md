...
| `diff` | show what changed between two branches/snapshots. `--from`/`--to` (or `--from-snapshot`/`--to-snapshot`) are required — there is no unbounded whole-graph diff. `--summary` prints per-op counts and affected types; without it you get a bounded page of changes (`--limit`, default 100) ordered by `(table_key, id)`, resumable with `--cursor`. Filter with `--types`/`--kinds`/`--ops`. Reports ids + operation (+ edge endpoints), never property values — read those with `query`. See [changes](../branching/changes.md) |
