...
| `blob get \| put \| stat <TYPE> <ID> <PROP>` | one row's `Blob` property. `get` streams the payload bytes to stdout (or `--out <path>`); `--offset`/`--length` request a bounded range; `--branch`/`--snapshot` (mutually exclusive) pick the read target. `put` uploads raw bytes from `--file <path>` or stdin into an **existing** row's cell (update-only — create the row first; ≤32 MiB minus row overhead; `--if-match <etag>` makes it conditional, failing with the current validator on mismatch; `--as` records the actor). `stat` prints kind/size/etag metadata with zero payload I/O (`--json` for the object). External references are never fetched: `stat` reports the stored URI, `get` fails with a typed error carrying it; `put` over one replaces it with the inline payload. Graph addressed via `--uri <store>`, `--store`, `--server` + `--graph`, or `--profile` |
