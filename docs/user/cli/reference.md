...
| `blob get \| stat <TYPE> <ID> <PROP>` | one row's `Blob` property. `get` streams the payload bytes to stdout (or `--out <path>`); `--offset`/`--length` request a bounded range; `--branch`/`--snapshot` (mutually exclusive) pick the read target. `stat` prints kind/size/etag metadata with zero payload I/O (`--json` for the object). External references are never fetched: `stat` reports the stored URI, `get` fails with a typed error carrying it. Graph addressed via `--uri <store>`, `--store`, `--server` + `--graph`, or `--profile` |
