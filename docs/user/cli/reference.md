...
| `queries list \| validate` | inspect a stored-query registry. `list` is dual-scope: a cluster's applied registry (`--cluster <dir\|uri>`; `--graph <id>` to scope one graph, omitted = all graphs merged) **or a served graph's catalog** (`--server <name\|url> --graph <id>`, reading `GET /queries` with the usual bearer token — the listing twin of by-name invocation). It prints each query's kind (read/mutation), name, typed params, and `[mcp: …]` exposure; a query's `@description`/`@instruction` are shown as indented `description:` / `instruction:` lines when declared (omitted otherwise). `--json` emits `{name, mcp_expose, tool_name, mutation, params}` plus `description`/`instruction` **only when present** — matching the HTTP `GET /queries` catalog ([server.md](../operations/server.md)). Served param types are synthesized from the wire's decomposed kinds, which are width-lossy (`I32`/`U32` both display as `Int`, etc.); the cluster arm shows exact GQ spellings. `validate` is cluster-only — it type-checks the registry against each graph's schema and exits non-zero on a broken query |
...
- **`control`** — `cluster *` via `--config <dir>`; `policy *` and `queries *` via `--cluster <dir|uri>` or a cluster profile. `queries list` is dual-scope: it also reads a served graph's catalog via `--server <name|url> --graph <id>` (or a server-bound profile/default), calling `GET /queries` with the usual bearer token.
