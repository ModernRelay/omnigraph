...
```bash
omnigraph mutate register_employee_with_team --query mutations.gq \
    --params '{"name":"Alice","age":30,"team":"Acme"}' --store graph.omni
```
...
```bash
# Query 1
omnigraph mutate register_employee --query mutations.gq \
    --params '{"name":"Alice","age":30}' --store graph.omni

# Query 2 — runs after Query 1 has already published
omnigraph mutate link_to_team --query mutations.gq \
    --params '{"name":"Alice","team":"Acme"}' --store graph.omni
```
...
omnigraph branch create --from main onboarding/2026-04-25 --store graph.omni
...
omnigraph mutate register_employee --branch onboarding/2026-04-25 \
    --query mutations.gq \
    --params '{"name":"Alice","age":30}' --store graph.omni

omnigraph mutate register_employee --branch onboarding/2026-04-25 \
    --query mutations.gq \
    --params '{"name":"Bob","age":25}' --store graph.omni

omnigraph mutate link_to_team --branch onboarding/2026-04-25 \
    --query mutations.gq \
    --params '{"name":"Alice","team":"Acme"}' --store graph.omni

# Inspect the branch — read queries work just like on main.
omnigraph query list_employees --branch onboarding/2026-04-25 \
    --query queries.gq --store graph.omni

# Happy with what's on the branch? Merge it. This is one atomic publish:
# `main` flips to include every commit on the branch.
omnigraph branch merge onboarding/2026-04-25 --into main --store graph.omni

# OR: not happy? Throw it away. `main` is untouched.
# omnigraph branch delete onboarding/2026-04-25 --store graph.omni
```
...
```bash
# Agent A
omnigraph branch create --from main agent-a/work --store graph.omni
omnigraph mutate <mutation-name> --query mutations.gq --branch agent-a/work … --store graph.omni
# … many mutations …
omnigraph branch merge agent-a/work --into main --store graph.omni

# Agent B (running concurrently)
omnigraph branch create --from main agent-b/work --store graph.omni
omnigraph mutate <mutation-name> --query mutations.gq --branch agent-b/work … --store graph.omni
# … many mutations …
omnigraph branch merge agent-b/work --into main --store graph.omni
```
