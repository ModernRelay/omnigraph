...
- **Reconciles pre-existing drift.** If a table's Lance HEAD is already ahead of its manifest pin (e.g. it was compacted by a pre-publish `optimize`, or a recovery `restore` commit advanced HEAD), `optimize` catches the manifest up to HEAD **even when there is nothing left to compact** — reported as `committed: true` with `fragments_added == fragments_removed == 0`. This is a metadata-only `__manifest` catch-up (no Lance HEAD advance, hence no sidecar), and because every drift source is content-preserving it never changes query results. A single `optimize` run is therefore the way to heal a graph that drifted under an older build.
