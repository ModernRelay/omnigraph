...
- **Layout**:
  ...
  - `_stream_tokens.lance` — graph-global RFC-026 current-token state. Its raw Lance HEAD is not authority; `__manifest` selects one exact witness. Current schema v3 includes versioned `DEAD_LETTERED` evidence.
  - `_graph_commit_recoveries.lance` — the graph-lineage crash-recovery audit log (see below). Recovery-v19/v21 authority retirement is lineage-neutral and instead uses its selected immutable receipt/profile chain, so it appends no row here. The former `_graph_commits.lance` / `_graph_commit_actors.lance` lineage tables are **retired**: lineage lives in `__manifest`, so a graph this binary creates has neither.
...
(`v19`). An earlier-stamped graph is rebuilt via export/import, not migrated in
place.
...
| v17 | Raises the sidecar ceiling to recovery-v19 for root-wide authority retirement. A stopped/offline, state-lock-held plan proves an exact `DISABLED`, all-`SEALED`, recovery-clear cut with base/token parity and at least one current `WITHDRAWN` token. Confirmation appends one immutable receipt and selects it with `RETIRED` without moving graph lineage. The source becomes permanently query/status/export-only; retired export emits the root receipt, a recomputable selected-member witness, and the ordered membership proof needed to recover the receipt-bound cut. |
| v18 | Raises the sidecar ceiling to recovery-v20 for exact `DataBlock` correction. V20 binds the blocked cut, base effect, combined token/correction/management-receipt effect, graph lineage, and complete next `DRAINING` lifecycle; only their exact joint outcome clears the matching block. The frozen v14 correction scaffold remains inactive. |
| v19 | Upgrades `_stream_tokens.lance` to schema v3 and raises the sidecar ceiling to recovery-v21. `DeadLetterFold` owns one conditionally created bounded canonical object plus exact base/token effects, publishes mixed or all-diverted outcomes in one lifecycle/lineage decision, and makes each losing key current `DEAD_LETTERED`; all-diverted uses a marker-only base transaction. Exact retry is valid while terminal authority remains current, and a fresh ordinary successor can restore `PRESENT`. `StreamAuthorityRetirementV2` binds exact `PRESENT | WITHDRAWN | DEAD_LETTERED` counts and the selected token cut. **The only version this binary serves.** |
