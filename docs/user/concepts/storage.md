...
(`v13`). An earlier-stamped graph is rebuilt via export/import, not migrated in
place.
...
| v13 | Preserves lifecycle-v3 and raises the sidecar ceiling to recovery-v15. The hidden `StreamResume` owner recovery-covers revision-fenced `SEALED → OPEN` resume and guarded `DRAINING → OPEN` abort, including the higher-epoch claim and terminal claim/management receipts. Rebind, correction/retirement, the `SEALED` maintenance bridge, public row ingress, and CLI/HTTP/OpenAPI lifecycle verbs remain inactive. **The only version this binary serves.** |
