...
| v17 | Raises the sidecar ceiling to recovery-v19 for root-wide authority retirement. A stopped/offline, state-lock-held plan proves an exact `DISABLED`, all-`SEALED`, recovery-clear cut with base/token parity and at least one current `WITHDRAWN` token. Confirmation appends one immutable receipt and selects it with `RETIRED` without moving graph lineage. The source becomes permanently query/status/export-only; retired export emits the root receipt, a recomputable selected-member witness, and the ordered membership proof needed to recover the receipt-bound cut. Correction and public row/lifecycle transports remain inactive. **The only version this binary serves.** |
