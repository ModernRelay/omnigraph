...
- **Traversal**: `$src EDGE_NAME { min, max? } $dst` — variable-length paths via hop bounds; default 1..1 if bounds omitted. Walk semantics: a self-loop edge makes its node its own neighbor (and its own reach result), while a multi-hop cycle back to the source is pruned rather than re-emitted.
