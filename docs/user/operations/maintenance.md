...
- Optimize plans under one schema/main/all-table envelope, writes one recovery record for the complete productive table set, runs per-table compact→reindex work in bounded parallelism, and publishes the resulting pointers together. A crash after every table effect rolls the batch forward on the next read-write open; a partial effect set is compensated before any pointer becomes graph-visible. The current recovery record is still the legacy loose maintenance format, so destructive recovery retains the documented single-writer-process boundary until exact Optimize provenance lands.
