...
- Returns per-dataset stats. The compatibility fields remain `table_key, fragments_removed, fragments_added, committed, skipped, manifest_version, lance_head_version, pending_indexes`; here legacy `manifest_version` is the published dataset version, while `lance_head_version` is its Lance HEAD version. `pending_indexes` lists any declared `@index` property the reconciler could not build this run, with the reason — for example, a vector property with no trainable vectors yet.
...
- Classifies drift by reading the dataset's transaction history from legacy `manifest_version + 1` through the current HEAD. Only fragment-reservation and rewrite (compaction) operations are verified maintenance. Semantic operations such as append, delete, update, merge, or missing transaction history are not auto-healed.
...
- Policy options `keep_versions` and `older_than` — at least one is required.
  ...
  cutoffs.
- Returns per-dataset stats through the compatibility fields `table_key, bytes_removed, old_versions_removed, error`.
