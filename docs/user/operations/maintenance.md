...
- **Persists the graph-index (traversal adjacency) artifact.** After the graph-visible publication and the internal compaction, `optimize` builds the full-catalog CSR/CSC adjacency and writes it to `__graph_index/csr-current.bin` under the graph root, so a cold process loads traversal topology with one read instead of scanning every edge table. The artifact is **derived and regenerable**: `optimize` is its only writer, queries only read it, every load verifies per-edge freshness stamps plus a payload digest, and any rejection falls back to the in-memory build — a missing, stale, or damaged artifact can slow a traversal, never wrong it. The write is best-effort (a failure is a warning, not an `optimize` error), is skipped when no edge dataset advanced and an artifact already exists, and there is exactly one such object per graph root, overwritten in place; `cleanup` does not manage it.
