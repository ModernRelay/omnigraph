...
- **Does not repair enrolled streaming drift.** Do not use `repair` to adopt an unexplained head for a streaming declaration: that would bypass its sequencing authority. Stop serving, preserve the affected root for investigation, and rebuild a fresh graph from the last verified clean export or backup.
...
- **Refuses uncovered main-table drift.** Every manifest-visible main version
  ...
  ordinary unenrolled drift with `omnigraph repair` before cleanup. For enrolled
  streaming drift, follow the preserve-and-rebuild posture above; version GC
  never guesses around either kind of drift.
