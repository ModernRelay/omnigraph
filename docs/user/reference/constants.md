...
| Internal manifest schema version | `INTERNAL_MANIFEST_SCHEMA_VERSION = 15` | strict RFC-026 strand. V15 preserves lifecycle-v3, recovery-v15 resume/guarded drain-abort, and recovery-v16 `SEALED` EnsureIndices while activating recovery-v17 for private, checked-runtime, same-binding `SEALED` Optimize. V14 and older graphs require export/init/load rebuild |
...
| Recovery-sidecar schema ceiling | `SIDECAR_SCHEMA_VERSION = 17` | v17 activates only exact hidden `StreamSealedOptimize`: the bounded Optimize maintenance plan plus complete prior/next SEALED rows, achieved physical HEADs, and rollback proof refresh, with no token effect, maintenance receipt, or caller operation ID. V16 remains exact hidden `StreamSealedEnsureIndices`; v15 remains exact hidden `StreamResume`. V14's three-field resume scaffold and its correction/retirement/ledger-maintenance/sealed-maintenance/rebind discriminators remain frozen and fail closed. Recovery-v13 remains exactly `StreamProfileChange`; historical recovery-v10 enrollment and recovery-v12 lifecycle-v2 fold meanings are not reinterpreted |
