...
| Internal manifest schema version | `INTERNAL_MANIFEST_SCHEMA_VERSION = 11` | strict RFC-026 strand; preserves v5 identity, v6 keyed fencing, v7 stream-lifecycle authority, v8's bounded B1 worker, v9's stream-config v3 / lifecycle state v2 / manifest-selected token authority / trusted attribution / recovery-v12 contract, and v10's graph-global `stream_profile` singleton plus reserved fold-attribution dead-letter slot. V11 replaces the boolean profile with protocol-v2 state/receipt/delegation authority and makes profile mutation an offline checked cluster-control operation. Under `ENABLED`, Mutation/Load/delete are checked-runtime-only; under `DISABLING`, they are closed, and BranchMerge is closed under both modes even with the runtime. V10 and older graphs require export/init/load rebuild |
...
| Recovery-sidecar schema ceiling | `SIDECAR_SCHEMA_VERSION = 13` | `StreamProfileChange` is the only emitted/accepted v13 discriminator in this bounded v11 tranche. It owns one exact token-ledger `ProfileManagementReceipt` transaction; the terminal manifest CAS selects its token witness and the next profile together. Unknown or later lifecycle variants fail closed. Recovery-v10 enrollment and recovery-v12 ordinary private fold payloads retain their exact meanings. V13 does not activate public ingress, production enrollment, drain, claim, or another lifecycle family; that activation requires another strict graph/recovery strand |
...
| Private RFC-026 B2 recovery-JSON ceiling | `B2_MAX_TOKEN_RECOVERY_JSON_BYTES = 33,554,432` (32 MiB) | bounded token rows embedded in one recovery-v12 ordinary-fold sidecar; v13 does not redefine this envelope |
