...
| Internal manifest schema version | `INTERNAL_MANIFEST_SCHEMA_VERSION = 17` | strict RFC-026 strand. V17 preserves lifecycle-v3 and recovery-v15/v16/v17/v18 resume, maintenance, and rebind families while activating recovery-v19 for terminal stream-authority retirement/export. V16 and older graphs require export/init/load rebuild |
...
| Recovery-sidecar schema ceiling | `SIDECAR_SCHEMA_VERSION = 19` | v19 activates only `StreamAuthorityRetirement`: one exact immutable retirement-receipt transaction and the sole lineage-neutral `DISABLED → RETIRED` manifest publication. V18 remains exact hidden `StreamRebind`; v17 remains exact hidden `StreamSealedOptimize`; v16 remains exact hidden `StreamSealedEnsureIndices`; v15 remains exact hidden `StreamResume`. V14's resume/correction/retirement/ledger-maintenance/sealed-maintenance/rebind scaffolds remain frozen and fail closed. Recovery-v13 remains exactly `StreamProfileChange`; historical recovery-v10 enrollment and recovery-v12 lifecycle-v2 fold meanings are not reinterpreted |
