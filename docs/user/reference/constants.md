...
| Internal manifest schema version | `INTERNAL_MANIFEST_SCHEMA_VERSION = 14` | strict RFC-026 strand. V14 preserves lifecycle-v3 and recovery-v15 resume/guarded drain-abort while activating recovery-v16 for private, checked-runtime, same-binding `SEALED` EnsureIndices. V13 and older graphs require export/init/load rebuild |
...
| Recovery-sidecar schema ceiling | `SIDECAR_SCHEMA_VERSION = 16` | v16 activates only exact hidden `StreamSealedEnsureIndices`: the recovery-v8 CreateIndex plan plus complete prior/next SEALED rows, with no token effect, maintenance receipt, or caller operation ID. V15 remains exact hidden `StreamResume`. V14's three-field resume scaffold and its correction/retirement/ledger-maintenance/sealed-maintenance/rebind discriminators remain frozen and fail closed. Recovery-v13 remains exactly `StreamProfileChange`; historical recovery-v10 enrollment and recovery-v12 lifecycle-v2 fold meanings are not reinterpreted |
