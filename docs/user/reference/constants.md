...
| Internal manifest schema version | `INTERNAL_MANIFEST_SCHEMA_VERSION = 10` | strict RFC-026 strand; preserves v5 identity, v6 keyed fencing, v7 stream-lifecycle authority, v8's bounded B1 worker, and v9's stream-config v3 / lifecycle state v2 / manifest-selected token authority / trusted attribution / recovery-v12 contract, then adds the §4.7 P1 graph-global `stream_profile` enablement singleton and reserves the fold-attribution dead-letter slot. V9 and older graphs require export/init/load rebuild |
| Stream-profile payload version | `STREAM_PROFILE_PROTOCOL_VERSION = 1` | exact-equality wire gate on the `stream_profile` singleton's metadata; genesis is disabled at `profile_revision = 1`, and every accepted flip advances the revision strictly |
