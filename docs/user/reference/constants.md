...
| Internal manifest schema version | `INTERNAL_MANIFEST_SCHEMA_VERSION = 16` | strict RFC-026 strand. V16 preserves lifecycle-v3 and the recovery-v15/v16/v17 resume and maintenance families while activating recovery-v18 for private physical rebind. V15 and older graphs require export/init/load rebuild |
...
| Recovery-sidecar schema ceiling | `SIDECAR_SCHEMA_VERSION = 18` | v18 activates only exact hidden `StreamRebind`: complete prior `SEALED` authority, a fresh enrollment/shard namespace, immutable binding plus fence-only claim receipts, and the exact next `SEALED` proof. V17 remains exact hidden `StreamSealedOptimize`; v16 remains exact hidden `StreamSealedEnsureIndices`; v15 remains exact hidden `StreamResume`. V14's three-field resume scaffold and its correction/retirement/ledger-maintenance/sealed-maintenance/rebind discriminators remain frozen and fail closed. Recovery-v13 remains exactly `StreamProfileChange`; historical recovery-v10 enrollment and recovery-v12 lifecycle-v2 fold meanings are not reinterpreted |
