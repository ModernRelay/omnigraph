...
| Internal manifest schema version | `INTERNAL_MANIFEST_SCHEMA_VERSION = 18` | strict RFC-026 strand. V18 preserves lifecycle-v3 and recovery-v15/v16/v17/v18/v19 resume, maintenance, rebind, and retirement families while activating recovery-v20 for exact `DataBlock` correction. V17 and older graphs require export/init/load rebuild |
...
| Recovery-sidecar schema ceiling | `SIDECAR_SCHEMA_VERSION = 20` | v20 activates only `StreamCorrection`: exact pre-minted base and combined token/receipt participants plus the sole lineage-bearing `DataBlock` release publication. V19 remains exact `StreamAuthorityRetirement`; v18 remains exact hidden `StreamRebind`; v17 remains exact hidden `StreamSealedOptimize`; v16 remains exact hidden `StreamSealedEnsureIndices`; v15 remains exact hidden `StreamResume`. V14's correction/resume/retirement/ledger-maintenance/sealed-maintenance/rebind scaffolds remain frozen and fail closed. Recovery-v13 remains exactly `StreamProfileChange`; historical recovery-v10 enrollment and recovery-v12 lifecycle-v2 fold meanings are not reinterpreted |
