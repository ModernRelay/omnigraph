...
- **Write diagnostics + destructive-write safety (RFC 0011 Decision 9).** Every
...
- **Operator aliases get their own namespace (`omnigraph alias <name>`).** A
  ...
  `omnigraph alias <name> [args]` (RFC 0011 Decision 4), so an alias can never
...
- **No-graph addressing lists candidates (RFC 0011 Decision 7).** When a scope
...
- **Invoke stored queries by name (RFC 0011 Decision 3).** `omnigraph query
...
### Embeddings: provider-independent (RFC 0012)
...
- **`query`/`mutate` no longer take a positional graph URI, `--uri`, or
  `--name`** (RFC 0011 D3). The positional is now the query name; address the
