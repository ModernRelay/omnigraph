...
- **Managed folders preserve explicit CLI targets.** An explicit server,
  profile, store, or supported cluster address keeps ordinary command behavior,
  including beside malformed folder context. Other graph commands, aliases,
  and local tools are no longer blocked by that context. Implicit managed
  query/mutate refuses when an environment profile or default target competes
  with the folder binding; select the intended target before retrying. Scoped
  managed credentials and offline authorization remain unchanged. See
...
- **Lance correctness fixes.** Includes stable-row-ID delete/index alignment,
  ...
  for exact `nearest` ranking. The h2 0.4 dependency line
  advances to 0.4.16;
...
- Public `ServerConfig`, `ServingSnapshot`, and `ResolvedActor` retain their
  construction contracts from before managed data-token support. Embedders
  using the interim root/trust fields migrate to the additive managed boot
  APIs in the [developer guide](../dev/control-plane.md#public-embedding-apis).
  Ordinary boot avoids the added signed-trust canonicalization step; managed
  boot keeps strict root verification and offline authorization. The binary
  flag and trust-file format are unchanged. Callers that inspected claims on
  the interim `ResolvedActor` use `verify_authenticated_at` and the returned
  `AuthenticatedActor::data_claims` instead; `verify_at` remains an identity
  projection.
