...
- **Typed storage failures (RFC-038).** The Rust API replaces the exhaustive
  ...
  decision, and no generic retry API is added. Genuine adapter and Lance
  storage diagnostics keep their complete historical display text. Arrow
  batch/shape failures owned by manifest machinery now map to internal errors,
  persisted Blob descriptor contradictions remain `BlobIntegrity`, and user
  query failures remain `DataFusion`. Generic storage errors still map to HTTP
  500, with no OpenAPI, graph-format, or manifest-schema migration.
