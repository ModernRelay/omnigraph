...
- The embedded `OmniError` enum adds `InitializationCommitted`,
  `InitializationIndeterminate`, and `InitializationClaimed`. Exhaustive Rust
  matches must handle the new variants. They preserve the distinction between
  a confirmed manifest commit, an outcome that cannot be classified, and an
  initialization root reserved by another attempt; no HTTP graph-create route
  is exposed and the OpenAPI schema is unchanged. `__init_claim.json` is a
  transient control object, not an internal-manifest or recovery-sidecar format
  change.
