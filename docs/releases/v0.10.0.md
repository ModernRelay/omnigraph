...
- **Search-ordered results are ranked by score, deterministically.**
  ...
  documented entity-id tie-break, including through multi-hop traversals.
  Observable result order can change on upgrade where scores tie (previously
  the storage engine's internal order leaked through), and `order … limit n`
  now runs a bounded top-k sort. `_distance` and `_score` become reserved
  property names at schema validation. The persisted traversal-adjacency
  artifact moves to format v2 (self-describing sections); existing v1
  artifacts are rejected at load, so the first traversals after upgrade pay
  the in-memory build until the next `optimize` rewrites the artifact.
...
- **Typed storage failures (RFC-038).** The Rust API replaces the exhaustive
  ...
  `Unknown`; only positive transport evidence becomes `Transient`. Lance 10 can
...
- **Entity changes, durable feed, and baselines.** RFC-030 C0–C3 now ships
