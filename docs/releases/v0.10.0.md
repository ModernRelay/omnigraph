...
- **Bounded ranked full-text scans.** A `bm25()` or `rrf()` ordering with a
  `limit` now reads only the top-scoring matches (a small multiple of the
  limit) instead of hydrating every matching entity. This fixes ranked reads
  with traversals failing at corpus scale with an Arrow offset overflow
  (#563) and makes join-free ranked reads proportionally faster. When
  traversals or filters leave the limit unfilled, the query automatically
  rescans without the bound, so result sets are never truncated; aggregate
  returns are never bounded. Inside `rrf()`, both rankings now contribute
  their top-scoring windows to the fusion (previously the vector ranking was
  already bounded while the full-text ranking was exhaustive), so fused tail
  ordering over large matched corpora can differ from v0.9.
