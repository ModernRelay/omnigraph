...
- **Canonical graph vocabulary.** Graph operations and their human-facing
  ...
  observable wording in help, errors, human output, and current documentation.
  Exact upstream/tabular terms and compatibility names such as `table_key`,
  `row_id`, `manifest_version`, `--table`, and `rows_loaded` retain their
  published spelling; no compatibility alias, wire field, or storage field is
  removed. The renamed schema-layout heading also retains the legacy
  `#table-layout` deep-link anchor.
...
- **Entity changes, durable feed, and baselines.** RFC-030 C0–C3 now ships
  ...
  surfaces. Pages are bounded by row and byte ceilings; opaque continuation
...
- The embedded `OmniError` enum adds `InitializationCommitted`,
  ...
  preserve the distinction between a confirmed manifest commit, an outcome
