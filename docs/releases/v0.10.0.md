...
- **Declarative benchmark telemetry.** The non-gating `omnigraph-bench` tool
  ...
  publish complete canonical run-record-v1 JSON from a clean release build.
  Records are content-addressed and reached through immutable invocation
  pointers; `archive verify` streams and rechecks identity and bytes. Parent
  and worker bytes must match; their attestation records the executable digest,
  ...
  controlled AWS build receipt and therefore cannot authorize a performance
  ...
  concurrent immutable-pointer publication. The JSON archive remains
  authority, so the projection can be deleted and reconstructed. Archive-mode
  ...
  reports, and AWS orchestration remain future slices.
