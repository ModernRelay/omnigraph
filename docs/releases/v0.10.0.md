...
- **Graph-level HTTP Blob delivery.** `GET` and explicit `HEAD` on
  ...
  datasets and per-table lanes are not part of the wire contract. CLI `blob
  get/stat` follows separately.
- **Named-branch Blob reads fail closed without an incarnation witness.** An
  ...
  instead of
  risking same-name/same-version branch ABA. Genuine inherited-main history
  remains eligible after the graph-snapshot proof; property/schema checks still
  apply.
