...
- **Lance correctness fixes.** Includes stable-row-ID delete/index alignment,
  ...
  for exact `nearest` ranking. IVF nearest scans retain Lance's adaptive
  one-partition minimum but cap its maximum at 20 by default, preventing the
  centroid-distance heuristic from expanding to every payload partition;
  `OMNIGRAPH_ANN_NPROBES` tunes that ceiling. Detectably short standalone and
  RRF vector scans retry once without the ceiling. The h2 0.4 dependency line
  advances to 0.4.16;
