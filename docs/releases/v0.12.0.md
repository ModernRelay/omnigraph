...
- **Exact merge receipts and conservative write outcomes.** Branch and GQ
  merges return their own published commit, or null for a no-op, without
  sampling a later target head. Optional source-deletion failures preserve
  that receipt and structured error details. CLI data-write failures retain
  structured details and add command-level outcome guidance; verified
  retry-safe refusals exit 75, while uncertain outcomes remain non-retryable
  failures and conditional mismatches retain exit 4. Graph HTTP requests do
  not redirect or retry implicitly. See [CLI outcomes](../user/cli/outcomes.md)
  and [merge results](../user/branching/merge.md).
...
- **Merge response fields.** JSON gains `commit` and optional
  `branch_delete_error_details`; older responses still decode. Rust callers
  constructing `BranchMergeOutput` literals must supply the new fields.
  Existing engine `branch_merge` methods retain their return types; the new
  receipt methods are additive. No storage format changes.
