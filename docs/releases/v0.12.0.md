...
- Cleanup retains detached staging unless a published pin and matching linear
  transaction prove it redundant. Uncertain staging defers GC only for the
  affected table; older files are not treated as proof of an abandoned writer.
- Lost acknowledgements resolve against the attempted graph commit even when
  another writer advances HEAD before readback.
