...
- **`omnigraph repair` reports foreign drift instead of healing drift.** A
  ...
  `--force --confirm` publish nothing on a v11 graph, and `cleanup` never
...
- **Every table write is a detached Lance commit, published once and never
  ...
  succeeds without reopening. No writer reads a table's linear HEAD, so a
  foreign linear commit blocks nothing; `omnigraph repair` reports it as
  `foreign_drift`. `ensure_indices` and the explicit full-text rebuild
  ...
  unreferenced. Schema apply follows: an existing-table rewrite is a
  detached commit published as a pin, an added type is a linear `v1` create
  whose leftover from a crashed attempt the retry reclaims, the staged
  schema contract records the graph commit that publishes it so the next
  read-write open installs or discards it, and no `SchemaApply` sidecar is
  written; a failure before the manifest commit returns the plain error, and
  only a failure after it reports `RecoveryRequired`, naming the published
  commit. Optimize follows: each table's compaction is one detached rewrite
...
- **Storage format v11; every existing graph needs one explicit upgrade.**
  Normal open now serves internal manifest schema v11 only. A v8, v9 or v10
  graph from the 0.11.x line is refused until `omnigraph upgrade <graph>`
  (run `--check` first) takes it to v11: the metadata-only v10 stamp step
  where the source needs it, then `detached-only-v10-to-v11`, which judges
  every pin on every live branch and refuses before any write when a foreign
  linear commit blocks one (finding `blocked_promotion`), promotes every
  pending v10 pin once (the last promotion the engine runs), reaps the
  copies that promotion proves, records `omnigraph.last_linear_version` on
  every registration of every live branch in one publication per branch,
  and restamps. It keeps every branch, every table byte and the graph's
  system column spellings, runs on standalone roots with every writer
  stopped, and refuses a graph with a pending recovery sidecar until the
  0.11.x binary that wrote it has resolved it; a cluster-managed graph has
  no route yet. v11 exists because a registration's
  `published_dataset_version` no longer names a Lance version: an older
  binary would open it and misread the table as reclaimed history, so the
  stamp refuses it before any open. Older binaries refuse v11 graphs. v10 is
  a conversion source, and `--to-format 10` an intermediate target for a
  compatible older executable, for this release only; both are removed at
  the next release boundary. `--to-format 9` is not a target: the
  system-column respelling that v9 used to mark is `omnigraph schema
  upgrade-system-columns` on a served graph and does not change the stamp;
  a legacy-vintage graph is born at v11 too. That upgrade has schema apply's
  shape: each table's rename commits detached and is published as a pin,
  the staged contract names the publishing graph commit, and no recovery
  sidecar is written; a run interrupted before its manifest commit leaves
  the graph unchanged and is simply rerun, and one interrupted after it is
  completed by the next read-write open. Test seam
  ...
  `V5 ↔ V9 Format Fence` CI job is now `V5 ↔ V10 Format Fence`. Downgrade
  is an export with this binary followed by `init` and `load` with the older
  one; history and branches are lost.
