...
- **Crash recovery for every writer.** Mutations, loads, schema applies,
  ...
  compensates on the next read-write open or live Full `refresh()`,
  all-or-nothing; ambiguity fails closed as `RecoveryRequired`.

- **Live served recovery and cancellation-independent writes.** Every
  configured graph remains visible with read/write readiness, transient opens
  retry under a bounded supervisor, and a served `RecoveryRequired` schedules
  Full recovery through the same engine handle. HTTP disconnect drops only the
  response receiver; the server-owned write retains admission and runs to a
  terminal result. This addition does not change the v6 storage format.
...
- **`graphs list` resolves against the server registry** and no longer
  requires `--graph`. The registry now reports every configured graph;
  configured-but-unavailable routes return 503 while unknown ids remain 404.
