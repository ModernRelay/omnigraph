...
- **Bounded graph-batch ingestion.** CLI, HTTP, and SDK bulk ingestion use the
  ...
  only after it is visible. The experimental RFC 0026 write-ahead path was
...
- RFC 0030 (graph change feed) is published with its first internal groundwork;
  no public feed API ships. RFC 0031 and RFC 0032 (cost and correctness
  harnesses) are drafts.
