...
- **Storage**: per node/edge table lifetime a separate Lance dataset; multi-dataset commits coordinated atomically through internal `__manifest` schema v5. Manifest ownership, OCC, recovery, and the identity-derived paths under `nodes/` and `edges/` use the immutable table pair, while `table_key` is only the current public alias.
...
| Stable schema + table identity | — | Persisted accepted SchemaIR v2 owns one graph identity domain and a shared monotonic no-reuse allocator for nonzero type, property, and table-incarnation IDs. Internal manifest schema v5 keys table registration/version/tombstone rows, OCC, recovery ownership, and both `nodes/{stable_table_id:016x}-{table_incarnation_id:016x}` and `edges/{stable_table_id:016x}-{table_incarnation_id:016x}` paths by the immutable table pair; `table_key` is a mutable alias. A pure type rename changes only that alias and keeps the same dataset/path/version/history; drop/re-add mints a new type/property identity, incarnation, path, and independent Lance version sequence. This binary serves v5 only; older graphs use export/import rebuild. |
