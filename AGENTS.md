...
| Change feeds | — | Tiered, bounded diff (RFC-029): `GET /diff/summary` + `GET /diff`, `omnigraph diff`, and SDK `diff_between` / `diff_commits` / `diff_summary_between`. Manifest fast path skips unchanged tables; changes are totally ordered by `(table_key, id)` with keyset `limit`/`cursor` paging; `from`/`to` are mandatory and both sides are `read`-gated. It is an **identity** feed — ids + op + edge endpoints, never property values (read those via query / `entity_at`). Update detection digests every column including `Vector`/`Blob`, so an embedding-only change is still an `Update`, but only a fixed-width digest is retained per row. `limit` bounds the response and stops further table walking; it does not bound work within one table |
