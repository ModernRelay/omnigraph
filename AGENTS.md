...
| System reference — established vocabulary, full operation catalog, key flows (mermaid) | [docs/dev/system.md](docs/dev/system.md) |
...
# Feature-gated suites (each is its own CI step, not part of the default run)
cargo test -p omnigraph-engine --features failpoints --test failpoints         # fault injection
cargo test -p omnigraph-engine --features failpoints --test memwal_stream      # RFC-026 private stream core
cargo test -p omnigraph-engine --features failpoints --test memwal_stream_cost gate_r0_ -- --nocapture
cargo test -p omnigraph-engine --features failpoints --test memwal_stream_cost b2a_ -- --nocapture
cargo test -p omnigraph-cluster --features failpoints --test failpoints        # cluster crash windows
...
This repo has CocoIndex project settings in `.cocoindex_code/settings.yml`. Use `ccc` when semantic search or structural AST search is faster than reading files manually. `ccc` is an external tool, not a workspace binary — if it isn't on `PATH`, fall back to `rg`/`Grep` rather than trying to install it mid-task.
...
| Stable schema + table identity | — | Persisted accepted SchemaIR v2 owns the graph identity domain; `(stable_table_id, table_incarnation_id)` keys manifest ownership, recovery, and the `nodes/`/`edges/` paths. `table_key` is a mutable alias — never infer identity from it, a path, a Lance version, or a field ID. Rename preserves identity; drop/re-add mints a new lifetime. **This binary serves internal schema v9 only** (older graphs rebuild via export/import). → [system.md §2.3](docs/dev/system.md) |
...
| Atomic single-dataset commits | ✅ | **Multi-table publish is three layers**, not one Lance primitive: (1) each participant's Lance effect, (2) one identity-aware `__manifest` CAS for graph visibility, (3) recovery for the gap between them. Every writer arms an identity-bearing `__recovery/{ulid}.json` intent before its first durable effect, proves its baseline under ordered gates, and confirms complete outcomes before publication; foreign or buried effects fail closed. Engine writes stay behind sealed `TableStorage` `stage_*` + `commit_staged`. Recovery is process-local — cross-process destructive recovery still needs a distributed fence. → [system.md §4.2](docs/dev/system.md), [writes.md](docs/dev/writes.md) |
| Compaction (`compact_files`) + reindex (`optimize_indices`) | ✅ | `omnigraph optimize` orchestrates every node/edge table under one graph-visibility envelope: a single recovery-v9 `SidecarKind::Optimize` intent pins the set before any HEAD moves, tasks run `compact_files` + `optimize_indices` + declared-missing index materialization without publishing independently, and one maintenance-class manifest CAS makes them visible together. It **commits even with no compaction work if index coverage is stale**; **refuses on an unrecovered graph**; **skips uncovered HEAD > manifest drift** with `DriftNeedsRepair`; and compacts blob-bearing tables. → [maintenance.md](docs/user/operations/maintenance.md) |
...
| BTREE / inverted (FTS) / vector indexes | ✅ | `@index`/`@key` declares *intent*; the physical index is derived state that never fails a logical op. One chokepoint (`build_indices_on_dataset_for_catalog`) dispatches by type — enum + orderable scalar → BTREE, free-text String → FTS, Vector → vector — idempotent and lazy across branches. **Schema apply, load, and mutate build no indexes inline**; `ensure_indices` materializes every declared-but-missing artifact for one table in one staged mixed CreateIndex transaction and reports untrainable Vector columns as pending. → [indexes.md](docs/user/search/indexes.md) |
| Strict insert / upsert ingestion | ✅ transaction conflict filters + uncommitted fragment staging | Every v9 node/edge table declares exact non-null physical `id` as Lance's unenforced PK. Strict insert and upsert both route through the sealed exact-`id`, forced-v2 MergeInsert adapter; bare Lance Append is test-only. Mutation/Load keeps one transaction per table and rejects more than 8,192 rows / 32 MiB before arm. BranchMerge's proven all-new route consumes a complete `omnigraph.insert_absence=v1` certificate chain to skip target preflights entirely, falling back to the general ordered diff whenever proof is missing or malformed. → [RFC-023](docs/rfcs/0023-key-conflict-fencing.md), [writes.md](docs/dev/writes.md) |
| MemWAL streaming foundation | ✅ MemWAL system index, durable WAL generations, epoch-fenced shard writer | Schema v9 carries the identity-keyed physical binding, lifecycle state-v2, stream-config v3, and the manifest-selected `_stream_tokens.lance` pointer. The **crate-private, feature-gated** B1/B2 core admits one bounded no-roll generation per writer (8,192 rows / 32 MiB), acknowledges only after the durability watcher *and* the same writer's post-durability `check_fenced()`, and folds through recovery-v12 into one manifest CAS. Profile is unbounded retain-all: no quota, no `_mem_wal` deletion, no generation GC. **Every production surface — enrollment, lifecycle, Cedar, SDK/CLI/HTTP/OpenAPI — is inactive.** → [system.md §2.6](docs/dev/system.md), [RFC-026](docs/rfcs/0026-memwal-streaming-ingest.md) |
